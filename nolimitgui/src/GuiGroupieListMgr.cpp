...
GuiGroupie* GuiGroupieListMgr::updateGroupie( VxNetIdent* hisIdent, EHostType hostType )
{
    ...
    GuiUser* guiUser = m_MyApp.getUserMgr().updateUser( hisIdent, hostType );
    if( user )
    {
...
GuiGroupie* GuiGroupieListMgr::updateGroupieInfo( GroupieInfo& groupieInfo )
{
    ...
    if( !guiGroupie )
    {
        GuiUser* guiUser = m_MyApp.getUserMgr().getUser( groupieInfo.getUserOnlineId() );
        // make a new one
        guiGroupie = new GuiGroupie( m_MyApp, guiUser, groupieInfo.getUserOnlineId(), groupieInfo );
...
void GuiGroupieListMgr::wantGroupieListCallbacks( GuiGroupieListCallback* client, bool enable )
{
    ...
    if( enable )
    {
        m_GroupieListClients.emplace_back( client );
    }
}
