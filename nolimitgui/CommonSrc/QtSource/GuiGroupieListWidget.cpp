...
void GuiGroupieListWidget::addOrUpdateUser( GroupieId& groupieId, GuiUserJoin* guiUserJoin )
{
    ...
    if( guiUser )
    {
        ...
        for( int iRow = 0; iRow < iCnt; iRow++ )
        {
            ...
            {
                GuiGroupieListSession* groupieSession = listItem->getHostSession();
                if( groupieSession && groupieSession->getGroupieId().getUserOnlineId() == guiUserJoin->getUser()->getMyOnlineId() )
                {
...
void GuiGroupieListWidget::updateUser( GuiUser* guiUser )
{
    ...
    for( int iRow = 0; iRow < iCnt; iRow++ )
    {
        ...
        {
            GuiGroupieListSession* groupieSession = listItem->getHostSession();
            if( groupieSession && groupieSession->getGroupieId().getUserOnlineId() == guiUser->getMyOnlineId() )
            {
...
//============================================================================
void GuiGroupieListWidget::callbackUserUpdated( GuiUser* guiUser )
{
    updateUser( guiUser );
}
