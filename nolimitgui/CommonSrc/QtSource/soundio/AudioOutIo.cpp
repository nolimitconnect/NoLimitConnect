...
bool AudioOutIo::initAudioOut( QAudioFormat& audioFormat, const QAudioDevice& deviceInfo )
{
    if( !m_initialized )
    {
        QAudioFormat tmpAudioFormat = audioFormat;
        tmpAudioFormat.setSampleRate( AUDIO_DEVICE_SAMPLE_RATE );
        tmpAudioFormat.setChannelCount( AUDIO_CHANNELS );
        tmpAudioFormat.setSampleFormat( QAudioFormat::Int16 );

        if( deviceInfo.isFormatSupported( tmpAudioFormat ) )
        {
            m_AudioFormat = tmpAudioFormat;
        }
        else
        {
            LogMsg( LOG_DEBUG, "AudioOutIo::initAudioOut failed set formt rate %d channels %d", AUDIO_DEVICE_SAMPLE_RATE, AUDIO_CHANNELS );
            m_AudioFormat = audioFormat;
        }

        setUpsampleMultiplier( (m_AudioFormat.sampleRate() * m_AudioFormat.channelCount()) / ECHO_SAMPLE_RATE );
        int deviceIndex = 0;
...
bool AudioOutIo::soundOutDeviceChanged( int deviceIndex )
{
    ...
    if( m_AudioIoMgr.getUseFixedAudioOutBufferSize() )
    {
        m_AudioOutputDevice->setBufferSize( AUDIO_BUF_SIZE );
    }
...
void AudioOutIo::slotUpdateSpeakerOutputEnable( void )
{
    ...
    {
        if( m_AudioOutputDevice->state() == QAudio::StoppedState )
        {
            startAudioOut();
        }
        else if (m_AudioOutputDevice->state() == QAudio::SuspendedState)
        {
...
{
    // setBufferSize must be called before device->start
    if( m_AudioIoMgr.getUseFixedAudioOutBufferSize() )
    {
        m_AudioOutputDevice->setBufferSize( 2048 );
    }
...
qint64 AudioOutIo::readData( char *data, qint64 maxlen )
{
    ...
    if( !maxlen )
    {
        ...
        LogMsg( LOG_ERROR, "AudioOutIo::readData has 0 maxlen. Consider not calling setBufferSize %d state %s error ", 
            m_AudioOutputDevice->bufferSize(), m_AudioIoMgr.describeAudioState( audioState ), m_AudioIoMgr.describeAudioError( audioError ) );
        return maxlen;
    }
