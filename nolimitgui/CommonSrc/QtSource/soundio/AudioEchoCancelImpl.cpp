...
//============================================================================
void AudioEchoCancelImpl::processEchoCancelThreaded( AudioSpeakerBuf& speakerProcessedBuf, QMutex& speakerProcessedMutex )
{
	...
	if( !m_AudioIoMgr.isSpeakerOutputWanted() )
	{
		...
		while( m_ProcessMicSamples.getSampleCnt() >= AUDIO_SAMPLES_PER_FRAME )
		{
			m_MicSamplesMutex.lock();
			...
			{
				m_AudioIoMgr.fromGuiEchoCanceledSamplesThreaded( m_ProcessMicSamples.getSampleBuffer(), AUDIO_SAMPLES_PER_FRAME, false );
			}

			m_ProcessMicSamples.samplesWereRead( AUDIO_SAMPLES_PER_FRAME );
			m_MicSamplesMutex.unlock();
		}
