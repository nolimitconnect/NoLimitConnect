...
bool AudioInIo::initAudioIn( QAudioFormat& audioFormat, const QAudioDevice& defaultDeviceInfo )
{
    if( !m_initialized )
    {
        ...
        setDivideSamplesCount( audioFormat.sampleRate() * audioFormat.channelCount() / ECHO_SAMPLE_RATE );
        int deviceIndex = 0;
...
bool AudioInIo::soundInDeviceChanged( int deviceIndex )
{
    ...
    format.setSampleRate( AUDIO_DEVICE_SAMPLE_RATE );
...
void AudioInIo::startAudio()
{
    if( m_initialized )
    {
        // setBufferSize must be called before start of device
        // m_AudioInputDevice->setBufferSize( AUDIO_BUF_SIZE );
        m_AudioInputDevice->setBufferSize( 2048 );
...
void AudioInIo::wantMicrophoneInput( bool enableInput )
{
    ...
    if( enableInput )
    {
        if( m_AudioInputDevice->state() == QAudio::StoppedState )
        {
            // start in pull mode.. qt will call writeData as needed for microphone input
            startAudioIn();
        }
        else if( m_AudioInputDevice->state() == QAudio::SuspendedState )
        {
...
{
    if( !m_AudioInDeviceIsStarted && !m_AudioInputDevice->isNull() )
    {
        // unlike speaker we always use fixed buffer size so the microphone writes are faster
        // set buffer size must be called before device->start
        m_AudioInputDevice->setBufferSize( AUDIO_BUF_SIZE );

        // start in pull mode.. qt will call readData as needed for sound output
        m_AudioInputDevice->start( this );

        m_AudioInState = m_AudioInputDevice->state();
        QAudio::Error audioError = m_AudioInputDevice->error();
        m_AudioInDeviceIsStarted = m_AudioInState != QAudio::StoppedState && audioError == QAudio::NoError;
        ...
        {
            LogMsg( LOG_DEBUG, "AudioInIo::startAudioIn failed state %s error %s ", m_AudioIoMgr.describeAudioState( m_AudioInState ), m_AudioIoMgr.describeAudioError( audioError ) );
        }
    }
}
