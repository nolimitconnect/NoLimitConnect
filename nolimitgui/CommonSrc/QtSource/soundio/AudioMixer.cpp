...
qint64 AudioMixer::readRequestFromSpeaker( char* data, qint64 maxlen )
{
    ...
    LogMsg( LOG_VERBOSE, "AudioMixer::readRequestFromSpeaker speaker buf lock" );
...
void AudioMixer::processAudioMixerThreaded( void )
{
    while( false == m_ProcessAudioMixerThread.isAborted() )
    {
        ...
        LogMsg( LOG_VERBOSE, "AudioMixer::processAudioMixerThreaded m_ProcessedBufMutex.lock" );
        m_ProcessedBufMutex.lock();
...
{
    m_MyApp.fromGuiAudioOutSpaceAvaiThreaded( AUDIO_BUF_SIZE );
}
