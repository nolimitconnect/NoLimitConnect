...
void AudioMixerFrame::clearFrame( bool fillSilence )
{
	...
	if( fillSilence )
	{
		...
		memset( m_SpeakerBuf, 0, sizeof( m_SpeakerBuf ) );
		m_SpeakerSamplesWrote = AUDIO_SAMPLES_PER_FRAME;
		m_IsProcessed = true;
	}
}
...
{
	return AUDIO_BUF_SIZE - (m_MixerSamplesWrote - m_MixerSamplesRead); // how many bytes is available to write into frame
}
...
int AudioMixerFrame::toMixerPcm8000HzMonoChannel( EAppModule appModule, int16_t* pcmData, bool isSilenceIn )
{
	...
	{
		AudioUtils::mixPcmAudio( (int16_t*)pcmData, (int16_t*)m_MixerBuf, AUDIO_BUF_SIZE );
		m_IsSilentSamples = false;
	}
...
void AudioMixerFrame::processFrameForSpeakerOutputThreaded( int16_t prevFrameSample )
{
	...
	if( m_InputIds.empty() )
	{
		...
		memset( m_MixerBuf, 0, AUDIO_BUF_SIZE );
		memset( m_SpeakerBuf, 0, AUDIO_BUF_SIZE );
		m_PeakValue0to100 = 0;
		return;
	}
	...
	{
		AudioUtils::upsamplePcmAudioLerpPrev( m_MixerBuf, AUDIO_SAMPLES_PER_FRAME, AUDIO_FRAME_TO_DEVICE_RATE_MULTIPLIER, prevFrameSample, m_SpeakerBuf );
		m_PeakValue0to100 = AudioUtils::peakPcmAmplitude0to100( m_MixerBuf, AUDIO_SAMPLES_PER_FRAME );
	}	
...
{
	return m_MixerBuf[ AUDIO_SAMPLES_PER_FRAME - 1 ];
}
