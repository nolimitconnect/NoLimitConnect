...
void RenderGlWidget::lastEnd( CGUIFontTTFQt * font )
{
    ...
    if( !font->m_vertexTrans.empty() )
    {
        ...
        for( size_t i = 0; i < font->m_vertexTrans.size(); i++ )
        {
            ...
            CRect clip = renderSystem->ClipRectToScissorRect( font->m_vertexTrans[ i ].m_clip );
            if( !clip.IsEmpty() )
            {
            ...
            glMatrixModview.Get().Translatef( font->m_vertexTrans[ i ].m_translateX, font->m_vertexTrans[ i ].m_translateY, font->m_vertexTrans[ i ].m_translateZ );
            getGlFunctions()->glUniformMatrix4fv( modelLoc, 1, GL_FALSE, glMatrixModview.Get() );

            // Bind the buffer to the OpenGL context's GL_ARRAY_BUFFER binding point
            getGlFunctions()->glBindBuffer( GL_ARRAY_BUFFER, font->m_vertexTrans[ i ].m_vertexBuffer->bufferHandle );

            // Do the actual drawing operation, split into groups of characters no
            // larger than the pre-determined size of the element array
            for( size_t character = 0; font->m_vertexTrans[ i ].m_vertexBuffer->size > character; character += ELEMENT_ARRAY_MAX_CHAR_INDEX )
            {
                size_t count = font->m_vertexTrans[ i ].m_vertexBuffer->size - character;
