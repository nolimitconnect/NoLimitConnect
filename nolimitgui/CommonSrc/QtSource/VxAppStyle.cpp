                                ...
                                const QWidget*			widget ) const
{
    ...
    {
        // TODO figure out why QLineEdit and QSpinBox palettes are changed from base palette
    ...
    if( control == CC_ComboBox )
    {
        ...
        if( drawArrow )
        {
            if( widget->isEnabled() )
            {
                ...
                if( ( readOnly.typeId() == QMetaType::Bool ) && ( readOnly == true ) )
                {
                    drawArrow = false;
                }
            }
    ...
    if( control == CC_ScrollBar )
    {
        if( const QStyleOptionSlider *scrollbar = qstyleoption_cast<const QStyleOptionSlider *>( option ) )
        {
            ...
            if( scrollbar->subControls & SC_ScrollBarGroove )
            {
                newScrollbar.rect = scrollbar->rect;
                newScrollbar.state = saveFlags;
                newScrollbar.rect = subControlRect( control, &newScrollbar, SC_ScrollBarGroove, widget );
                if( newScrollbar.rect.isValid() )
                {
            ...
            if( scrollbar->subControls & SC_ScrollBarSlider )
            {
                ...
                if( newScrollbar.rect.isValid() )
                {
                    ...
                    // set the scrollbar scroll bar button color
                    //QBrush brushColor = newScrollbar.palette.highlight(); // burnt orange if Dark Theme is selected
                    QBrush brushColor = newScrollbar.palette.midlight();
                    QColor scrollButtonColor( brushColor.color() );

                    newScrollbar.palette.setBrush( QPalette::Active, QPalette::Button, scrollButtonColor );
                    newScrollbar.palette.setBrush( QPalette::Inactive, QPalette::Button, scrollButtonColor );
                    newScrollbar.palette.setBrush( QPalette::Disabled, QPalette::Button, scrollButtonColor );
                    newScrollbar.palette.setBrush( QPalette::Normal, QPalette::Button, scrollButtonColor );
                             ...
                             const QWidget*		    widget ) const
{
    ...
    case PM_ScrollBarExtent:
        return 2 * GuiParams::getControlIndicatorWidth();
