...
void GuiHostedListItem::updateWidgetFromInfo( void )
{
    ...
    if( guiUser && m_MyApp.getEngine().getMyOnlineId() == hostSession->getHostedId().getHostOnlineId() )
    {
    ...
    if( !ui.m_IconButton->hasImage() && guiUser )
    {
        ...
        {
            ui.m_IconButton->setIcon( m_MyApp.getMyIcons().getHostTypeClientIcon( hostSession->getHostType() ) );
        }
    }
