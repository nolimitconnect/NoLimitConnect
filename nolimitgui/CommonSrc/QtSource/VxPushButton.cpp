...
VxPushButton::VxPushButton( const QString& text, QWidget* parent )
    ...
    , m_EyeOverlayLastIconColor( m_MyApp.getAppTheme().getAcceptColor() )
...
//============================================================================
void VxPushButton::setEyeOverlayEnabled( bool enabled, EMyIcons eNotifyIcon )
{
    m_EyeOverlayEnabled = enabled;
    if( enabled )
    {
        if( eMyIconNone != eNotifyIcon )
        {
            m_EyeOverlayIcon = eNotifyIcon;
        }
    }
...
#define IMAGE_PADDING  2
    ...
    if( m_EyeOverlayEnabled && (eMyIconNone != m_EyeOverlayIcon) )
    {
        if( m_EyeOverlayImage.isNull()
            || (m_EyeOverlayColor != m_EyeOverlayLastIconColor)
            || (drawRect.size() != m_EyeOverlayLastIconSize)
            || (m_EyeOverlayIcon != m_LastEyeOverlayIcon) )
        {
            if( m_EyeOverlayColor != QColor( COLOR_TRANSPARENT ) )
            {
                m_EyeOverlayImage = getMyIcons().getIconPixmap( m_EyeOverlayIcon, drawRect.size(), m_EyeOverlayColor );
                m_EyeOverlayLastIconColor = m_EyeOverlayColor;
            }
            else
            {
                m_EyeOverlayImage = getMyIcons().getIconPixmap( m_EyeOverlayIcon, drawRect.size() );
            }
           
            if( !m_EyeOverlayImage.isNull() )
            {
                m_LastEyeOverlayIcon = m_EyeOverlayIcon;
                m_EyeOverlayLastIconSize = drawRect.size();
            }
        }
...
void VxPushButton::setPushToTalkStatus( EPushToTalkStatus pushToTalkStatus )
{
    ...
    m_PushToTalkStatus = pushToTalkStatus;
