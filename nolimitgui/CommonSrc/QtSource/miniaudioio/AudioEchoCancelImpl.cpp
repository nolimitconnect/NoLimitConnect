...
//============================================================================
void AudioEchoCancelImpl::echoCancelStartup( int sampleFreq )
{
	...
	int ref_sample_rate = sampleFreq;
	int ref_channels = 1;
	int rec_sample_rate = sampleFreq;
	int rec_channels = 1;
...
{
#if defined(USE_SPEEX_ECHO_CANCEL)
	resetSpeex( ECHO_SAMPLES_PER_FRAME );
#endif // defined(USE_SPEEX_ECHO_CANCEL)
}

//============================================================================
void AudioEchoCancelImpl::speakerReadSamples( int16_t* speakerReadData, int sampleCnt )
{
	...
	m_SpeakerLastCallSampleCnt = sampleCnt;
	m_TailSpeakerSamplesMs = m_SpeakerLastCallTimestamp + AudioUtils::audioDurationMs( ECHO_SAMPLE_RATE, sampleCnt );
	m_HeadSpeakerSamplesMs = m_TailSpeakerSamplesMs - AudioUtils::audioDurationMs( ECHO_SAMPLE_RATE, m_SpeakerSamples.getSampleCnt() );

	m_SpeakerSamplesMutex.unlock();
}

//============================================================================
void AudioEchoCancelImpl::micWroteSamples( int16_t* micWriteData, int sampleCnt )
{
	m_MicSamplesMutex.lock();
	m_MicSamples.writeSamples( micWriteData, sampleCnt );

	m_MicLastCallTimestamp = GetHighResolutionTimeMs();
	m_MicLastCallSampleCnt = sampleCnt;
	m_TailMicSamplesMs = m_MicLastCallTimestamp + AudioUtils::audioDurationMs( ECHO_SAMPLE_RATE, sampleCnt );
	m_HeadMicSamplesMs = m_TailMicSamplesMs - AudioUtils::audioDurationMs( ECHO_SAMPLE_RATE, m_MicSamples.getSampleCnt() );

	m_MicSamplesMutex.unlock();
}

//============================================================================
void AudioEchoCancelImpl::processEchoCancelThreaded( void )
{
	...
	int speakerEchoFrames = m_SpeakerSamples.getSampleCnt() / ECHO_SAMPLES_PER_FRAME;
	int micEchoFrames = m_MicSamples.getSampleCnt() / ECHO_SAMPLES_PER_FRAME;
	if( micEchoFrames )
	{
		if( !speakerEchoFrames )
		{
			// just send as if were processed
			int samplesToSend = micEchoFrames * ECHO_SAMPLES_PER_FRAME;
			m_AudioIoMgr.addEchoCanceledSamples( m_MicSamples.getSampleBuffer(), samplesToSend );
			m_MicSamples.samplesWereRead( samplesToSend );
			m_HeadMicSamplesMs = m_HeadMicSamplesMs + AudioUtils::audioDurationMs( ECHO_SAMPLE_RATE, samplesToSend );
		}
		else
		{
			int echoFramesToProcess = std::min( speakerEchoFrames, micEchoFrames );
			int samplesProcessed = echoFramesToProcess * ECHO_SAMPLES_PER_FRAME;

			int16_t* micSamples = m_MicSamples.getSampleBuffer();
			int16_t* speakerSamples = m_SpeakerSamples.getSampleBuffer();
			int16_t* echoCanceledBuf = new int16_t[ samplesProcessed ];
			int16_t* echoProcessBuf = echoCanceledBuf;
			for( int i = 0; i < echoFramesToProcess; i++ )
			{
				processEchoCancelFrame( micSamples, speakerSamples, ECHO_SAMPLES_PER_FRAME, echoProcessBuf );

				micSamples += ECHO_SAMPLES_PER_FRAME;
				speakerSamples += ECHO_SAMPLES_PER_FRAME;
				echoProcessBuf += ECHO_SAMPLES_PER_FRAME;
			}

			m_AudioIoMgr.addEchoCanceledSamples( echoCanceledBuf, samplesProcessed );

			m_MicSamples.samplesWereRead( samplesProcessed );
			m_HeadMicSamplesMs = m_HeadMicSamplesMs + AudioUtils::audioDurationMs( ECHO_SAMPLE_RATE, samplesProcessed );

			m_SpeakerSamples.samplesWereRead( samplesProcessed );
			m_HeadSpeakerSamplesMs = m_HeadSpeakerSamplesMs + AudioUtils::audioDurationMs( ECHO_SAMPLE_RATE, samplesProcessed );

			delete[] echoCanceledBuf;
		}
	}
	...
	m_ProcessSpeakerSamples.writeSamples( speakerProcessedBuf.getSampleBuffer(), speakerProcessedBuf.getSampleCnt() );
	speakerProcessedBuf.clear();
	...
	if( m_ProcessSpeakerSamples.getSampleCnt() >= AUDIO_SAMPLES_PER_FRAME && m_ProcessMicSamples.getSampleCnt() >= AUDIO_SAMPLES_PER_FRAME )
	{
		m_EchoCancelInSync = attemptEchoSyncThreaded();
		if( m_EchoCancelInSync )
		{
			if( m_EchoCancelNeedsReset )
			{
				...
				resetEchoCanceler( ECHO_SAMPLES_PER_FRAME );
			}
			...
			int framesToProcess = std::min( m_ProcessMicSamples.getSampleCnt() / ECHO_SAMPLES_PER_FRAME, m_ProcessSpeakerSamples.getSampleCnt() / ECHO_SAMPLES_PER_FRAME );
			framesToProcess = std::min( framesToProcess, m_EchoCanceledSamples.freeSpaceSampleCount() / ECHO_SAMPLES_PER_FRAME );
			// not sure this is neccesary.. limit processing to one mixer buffer size
			framesToProcess = std::min( framesToProcess, AUDIO_SAMPLES_PER_FRAME / ECHO_SAMPLES_PER_FRAME );

			if( framesToProcess )
			{
				int samplesToProcess = framesToProcess * ECHO_SAMPLES_PER_FRAME;

				int samplesProcessedMs = AudioUtils::audioDurationMs( v, samplesToProcess );
				...
				{
					processEchoCancelFrame( micBuf, speakerBuf, ECHO_SAMPLES_PER_FRAME, echoCanceledBuf );

					micBuf += ECHO_SAMPLES_PER_FRAME;
					speakerBuf += ECHO_SAMPLES_PER_FRAME;
					echoCanceledBuf += ECHO_SAMPLES_PER_FRAME;
				}
			...
			if( m_EchoCanceledSamples.getSampleCnt() >= AUDIO_SAMPLES_PER_FRAME )
			{
				if( m_AudioIoMgr.getIsMicrophoneMuted() )
				{
					m_AudioIoMgr.fromGuiEchoCanceledSamplesThreaded( m_QuietSamplesBuf, AUDIO_SAMPLES_PER_FRAME, true );
				}
				else
				{
					m_AudioIoMgr.fromGuiEchoCanceledSamplesThreaded( m_EchoCanceledSamples.getSampleBuffer(), AUDIO_SAMPLES_PER_FRAME, false );
				}
				
				m_EchoCanceledSamples.samplesWereRead( AUDIO_SAMPLES_PER_FRAME );
			}
		}
		else
		{
...
bool AudioEchoCancelImpl::attemptEchoSyncThreaded( void )
{
	...
	if( !inSync )
	{
		...
		if( targetSpeakerTimeMs >= m_HeadSpeakerSamplesMs && targetSpeakerTimeMs <= m_TailSpeakerSamplesMs - 100 )
		{
			...
			int samplesToStrip = AudioUtils::audioSamplesRequiredForGivenMs( ECHO_SAMPLE_RATE, (int)(targetSpeakerTimeMs - m_HeadSpeakerSamplesMs) );
	...
	if( sampleCnt > 10 )
	{
		if( !m_SpeexState || sampleCnt != m_MicSamplesPerWrite )
		{
			...
			if( totalFrames > 7 && totalFrames < 12 )
			{
				...
				if( 100 < AudioUtils::audioDurationMs( ECHO_SAMPLE_RATE, 8 * sampleCnt ) )
				{
					totalFrames = 8;
				}
			}
...
//============================================================================
void AudioEchoCancelImpl::resetSpeakerBuffers( void )
{
