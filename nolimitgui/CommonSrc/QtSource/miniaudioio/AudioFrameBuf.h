...
{
public:
	static const int MAX_SAMPLES_IN_BUFFER = AUDIO_SAMPLES_PER_FRAME; // can hold up to one audio frame

	AudioFrameBuf() = default;
	AudioFrameBuf( const AudioFrameBuf& rhs ) = delete;
	~AudioFrameBuf() = default;

	AudioFrameBuf& operator = ( const AudioFrameBuf& rhs ) = delete;
	...
	int16_t					    m_PcmData[ MAX_SAMPLES_IN_BUFFER ];

	int							m_MaxSamples{ MAX_SAMPLES_IN_BUFFER };
	int							m_SampleCnt{ 0 };
};
