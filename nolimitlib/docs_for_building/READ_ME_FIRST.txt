...
If building with Visual Studio then
	1.) Open $(SRC_ROOT)/nolimitgui/build/VS2019/NoLimitConnectAll.sln
	2.) Rebuild All
	
If building with Qt for Andorid
	1.) Repeat the following steps for BuildNlc1_NlcDependLibs.pro, BuildNlc2_NlcCore.pro and NoLimitConnectApp.pro
...
If building with Qt for Linux
	1.) If a low resource machine then Open and build BuildNlc1_NlcDependLibs.pro, BuildNlc2_NlcCore.pro and NoLimitConnectApp.pro
	2.) If a high resource machine then  Open and build BuildNoLimitConnect_All.pro
