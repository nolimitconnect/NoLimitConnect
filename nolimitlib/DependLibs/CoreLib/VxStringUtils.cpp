...
const std::string VxStringUtils::Empty = "";
std::string VxStringUtils::m_lastUUID = "";
...
wstring VxStringUtils::Format(const wchar_t *fmt, ...)
{
  va_list args;
  va_start(args, fmt);
...
wstring VxStringUtils::FormatV(const wchar_t *fmt, va_list args)
{
  if (fmt == NULL)
    return L"";
...
int VxCompareWchar (const void* a, const void* b)
{
...
wchar_t VxToLowerUnicode(const wchar_t& c)
{
  wchar_t* p = (wchar_t*) bsearch (&c, unicode_uppers, sizeof(unicode_uppers) / sizeof(wchar_t), sizeof(wchar_t), VxCompareWchar );
  if (p)
...
wchar_t VxToUpperUnicode(const wchar_t& c)
{
  wchar_t* p = (wchar_t*) bsearch (&c, unicode_lowers, sizeof(unicode_lowers) / sizeof(wchar_t), sizeof(wchar_t), VxCompareWchar );
  if (p)
...
void VxStringUtils::ToUpper(string &str)
{
  std::transform(str.begin(), str.end(), str.begin(), ::toupper);
}

void VxStringUtils::ToUpper(wstring &str)
{
  transform(str.begin(), str.end(), str.begin(), VxToUpperUnicode );
}
...
bool VxStringUtils::EqualsNoCase(const std::string &str1, const std::string &str2)
{
...
int VxStringUtils::CompareNoCase(const std::string &str1, const std::string &str2)
{
...
string VxStringUtils::Left(const string &str, size_t count)
{
  count = max((size_t)0, min(count, str.size()));
  return str.substr(0, count);
}

string VxStringUtils::Mid(const string &str, size_t first, size_t count /* = string::npos */)
{
...
string VxStringUtils::Right(const string &str, size_t count)
{
  count = max((size_t)0, min(count, str.size()));
  return str.substr(str.size() - count);
}

std::string& VxStringUtils::Trim(std::string &str)
{
  TrimLeft(str);
  return TrimRight(str);
}

std::string& VxStringUtils::Trim(std::string &str, const char* const chars)
{
  TrimLeft(str, chars);
...
std::string& VxStringUtils::TrimLeft(std::string &str)
{
  str.erase(str.begin(), ::find_if(str.begin(), str.end(), std::not1(std::function<int(char)>(isspace_c))));
  return str;
}

std::string& VxStringUtils::TrimLeft(std::string &str, const char* const chars)
{
...
std::string& VxStringUtils::TrimRight(std::string &str)
{
  str.erase(::find_if(str.rbegin(), str.rend(), std::not1(std::function<int(char)>(isspace_c))).base(), str.end());
  return str;
}

std::string& VxStringUtils::TrimRight(std::string &str, const char* const chars)
{
...
std::string& VxStringUtils::RemoveDuplicatedSpacesAndTabs(std::string& str)
{
...
int VxStringUtils::Replace(string &str, char oldChar, char newChar)
{
  int replacedChars = 0;
...
int VxStringUtils::Replace(std::string &str, const std::string &oldStr, const std::string &newStr)
{
  if (oldStr.empty())
    return 0;
...
int VxStringUtils::Replace(std::wstring &str, const std::wstring &oldStr, const std::wstring &newStr)
{
  if (oldStr.empty())
    return 0;
...
bool VxStringUtils::StartsWith(const std::string &str1, const std::string &str2)
{
...
bool VxStringUtils::StartsWithNoCase(const std::string &str1, const std::string &str2)
{
...
bool VxStringUtils::EndsWith(const std::string &str1, const std::string &str2)
{
...
bool VxStringUtils::EndsWithNoCase(const std::string &str1, const std::string &str2)
{
  if (str1.size() < str2.size())
    return false;
...
std::string VxStringUtils::Join(const vector<string> &strings, const std::string& delimiter)
{
  std::string result;
...
vector<string> VxStringUtils::Split(const std::string& input, const std::string& delimiter, unsigned int iMaxStrings /* = 0 */)
{
...
int VxStringUtils::FindNumber(const std::string& strInput, const std::string &strFind)
{
...
int64_t VxStringUtils::AlphaNumericCompare(const wchar_t *left, const wchar_t *right)
{
...
int VxStringUtils::DateStringToYYYYMMDD(const std::string &dateString)
{
  vector<string> days = VxStringUtils::Split(dateString, "-");
  if (days.size() == 1)
...
long VxStringUtils::TimeStringToSeconds(const std::string &timeString)
{
  std::string strCopy(timeString);
  VxStringUtils::Trim(strCopy);
  if(VxStringUtils::EndsWithNoCase(strCopy, " min"))
  {
  ...
  {
    vector<string> secs = VxStringUtils::Split(strCopy, ":");
    int timeInSecs = 0;
...
std::string VxStringUtils::SecondsToTimeString(long lSeconds, VX_TIME_FORMAT format)
{
  ...
  if (format == VX_TIME_FORMAT_GUESS)
    format = (hh >= 1) ? VX_TIME_FORMAT_HH_MM_SS : VX_TIME_FORMAT_MM_SS;
  std::string strHMS;
  if (format & VX_TIME_FORMAT_HH)
    strHMS += VxStringUtils::Format("%02.2i", hh);
  else if (format & VX_TIME_FORMAT_H)
    strHMS += VxStringUtils::Format("%i", hh);
  if (format & VX_TIME_FORMAT_MM)
    strHMS += VxStringUtils::Format(strHMS.empty() ? "%02.2i" : ":%02.2i", mm);
  if (format & VX_TIME_FORMAT_SS)
    strHMS += VxStringUtils::Format(strHMS.empty() ? "%02.2i" : ":%02.2i", ss);
  return strHMS;
}

bool VxStringUtils::IsNaturalNumber(const std::string& str)
{
  size_t i = 0, n = 0;
...
bool VxStringUtils::IsInteger(const std::string& str)
{
  size_t i = 0, n = 0;
  // allow whitespace,-,digits,whitespace
  while (i < str.size() && isspace((unsigned char) str[i]))
...
int VxStringUtils::asciidigitvalue(char chr)
{
  if (!isasciidigit(chr))
    return -1;
...
int VxStringUtils::asciixdigitvalue(char chr)
{
...
void VxStringUtils::RemoveCRLF(std::string& strLine)
{
  VxStringUtils::TrimRight(strLine, "\n\r");
}

std::string VxStringUtils::SizeToString(int64_t size)
{
  std::string strLabel;
  ...
  if (!i)
    strLabel = VxStringUtils::Format("%.0lf %cB ", s, prefixes[i]);
  else if (s >= 100.0)
    strLabel = VxStringUtils::Format("%.1lf %cB", s, prefixes[i]);
  else
    strLabel = VxStringUtils::Format("%.2lf %cB", s, prefixes[i]);

  return strLabel;
}

// return -1 if not, else return the utf8 char length.
int VxIsUTF8Letter(const unsigned char *str)
{
  // reference:
...
size_t VxStringUtils::FindWords(const char*str, const char*wordLowerCase)
{
  ...
  do
  {
    ...
    else if ((l = VxIsUTF8Letter(s)) > 0)
    {
      s += l;
      while ((l = VxIsUTF8Letter(s)) > 0) s += l;
    }
    else
      ++s;
    while (*s && *s == ' ') s++;
...
int VxStringUtils::FindEndBracket(const std::string &str, char opener, char closer, int startPos)
{
  int blocks = 1;
...
void VxStringUtils::WordToDigits(std::string &word)
{
  static const char word_to_letter[] = "22233344455566677778889999";
  VxStringUtils::ToLower(word);
...
std::string VxStringUtils::CreateUUID()
{
...
bool VxStringUtils::ContainsKeyword(const std::string &str, const vector<string> &keywords)
{
...
std::string VxStringUtils::Paramify(const std::string &param)
{
  std::string result = param;
  // escape backspaces
  VxStringUtils::Replace(result, "\\", "\\\\");
...
std::vector<std::string> VxStringUtils::Tokenize(const std::string &input, const std::string &delimiters)
{
...
void VxStringUtils::Tokenize(const std::string& input, std::vector<std::string>& tokens, const std::string& delimiters)
{
  tokens.clear();
