...
//============================================================================
std::string VxGUID::toAsci( void ) const
{
    char lclBuf[ 33 ];
    int bufIdx = 0;
    int8_t* guidPart = (int8_t*)&m_u64HiPart;
    for( int i = 0; i < 8; i++ )
    {
        lclBuf[ bufIdx ] = *guidPart;
        guidPart++;
        bufIdx++;
    }

    guidPart = (int8_t*)&m_u64LoPart;
    for( int i = 0; i < 8; i++ )
    {
        lclBuf[ bufIdx ] = *guidPart;
        guidPart++;
...
//============================================================================
bool VxGUID::fromRawData( uint8_t* rawData )
{
    uint8_t* guidPart = (uint8_t * )&m_u64HiPart;
    for( int i = 0; i < 8; i++ )
    {
        *guidPart = *rawData;
        guidPart++;
