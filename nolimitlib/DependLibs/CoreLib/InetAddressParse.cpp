...
bool ParseIPv4OrIPv6_2 ( const char** ppszText, 
        unsigned char* abyAddr, int* pnPort, int* pbIsIPv6 )
{
    ...
    if ( ! bIsIPv6local )   //try to parse as IPv4
    {
        ...
        *(pbyAddrCursor++) = (unsigned char) nVal;
        ...
        {
            if ( NULL != pnPort )
                *pnPort = true;    //indicate we have no port part
            return 1;
        }
    }
        ...
        for ( nIdx = 0; nIdx < 8; ++nIdx )  //we've got up to 8 of these, so we will use a loop
        {
            ...
            else
            {
                if ( '.' == **ppszText )    //special case of ipv4 convenience notation
                {
                    ...
                    int bIsIPv6local;
                    int bParseResultlocal = ParseIPv4OrIPv6_2 ( &pszTextlocal, abyAddrlocal, NULL, &bIsIPv6local );
...
bool ParseIPv4OrIPv6( const char* pszText, unsigned char* abyAddr, uint16_t& retPort, bool& retIsIPv6 )
{
    const char* pszTextLocal = pszText;
    int pnPort = 0;
    int pbIsIPv6 = 0;
    if( ParseIPv4OrIPv6_2( &pszTextLocal, abyAddr, &pnPort, &pbIsIPv6 ) )
    {
...
{
    EIpAddrType addrType = VxGetIpAddrType( ipAddr );

    return addrType != eIpAddrTypeUnknown;
}

//============================================================================
void ip_parse_testcase ( const char* pszTest )
{
    ...
    LogMsg( LOG_DEBUG, "Test case '%s'", pszTest );
    const char* pszTextCursor = pszTest;
    bSuccess = ParseIPv4OrIPv6_2 ( &pszTextCursor, abyAddr, &nPort, &bIsIPv6 );
    if ( ! bSuccess )
    {
...
void TestsParseIp( void )
{
    ...
    ip_parse_testcase ( "127.0.0.1" );
    
    ip_parse_testcase ( "0000:0000:0000:0000:0000:0000:0000:0001/128" );

    //The "localhost" IPv4 address, with a specified port (80)
    ip_parse_testcase ( "127.0.0.1:80" );
    //The "localhost" IPv6 address
    ip_parse_testcase ( "::1" );
    //The "localhost" IPv6 address, with a specified port (80)
    ip_parse_testcase ( "[::1]:80" );
    //Rosetta Code's primary server's public IPv6 address
    ip_parse_testcase ( "2605:2700:0:3::4713:93e3" );
    //Rosetta Code's primary server's public IPv6 address, with a specified port (80)
    ip_parse_testcase ( "[2605:2700:0:3::4713:93e3]:80" );
    
    //ipv4 space
    ip_parse_testcase ( "::ffff:192.168.173.22" );
    //ipv4 space with port
    ip_parse_testcase ( "[::ffff:192.168.173.22]:80" );
    ...
    ip_parse_testcase ( "[::]:80" );

    ip_parse_testcase( "2604:980:7003:e6:29e::1" );
    ip_parse_testcase( "[2604:980:7003:e6:29e::1]:45124" );
