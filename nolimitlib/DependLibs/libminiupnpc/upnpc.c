                         ...
                         struct IGDdatas * data)
{
	...
	if(uptime > 0) {
		...
		LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "  Time started : %s", ctime(&timestarted));
	}
                             ...
                             struct IGDdatas * data)
{
	...
	LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "PortMappingNumberOfEntries : %u\n", num);*/
	LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, " i protocol exPort->inAddr:inPort description remoteHost leaseTime\n");
	do {
                                ...
                                struct IGDdatas * data)
{
	...
	if(r == UPNPCOMMAND_SUCCESS)
	{
		...
		{
			LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "%2d %s %5hu->%s:%-5hu '%s' '%s' %u\n",
	...
	{
		LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "GetListOfPortMappings() returned %d (%s)\n",
		       r, strupnperror(r));
	}
	r = UPNP_GetListOfPortMappings(urls->controlURL,
                                   data->first.servicetype,
			       ...
			       int addAny)
{
	...
	if(r!=UPNPCOMMAND_SUCCESS) {
		...
		LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "InternalIP:Port = %s:%s\n", intClient, intPort);
		LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "external %s:%s %s is redirected to internal %s:%s (duration=%s)\n",
               ...
               const char * remoteHost)
{
	...
	if(!proto)
	{
		LogCModule( MODULE_PORT_FORWARD, LOG_ERROR, "protocol invalid\n");
		return -1;
	}
...
static void GetFirewallStatus(struct UPNPUrls * urls, struct IGDdatas * data)
{
	...
	LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "FirewallEnabled: %d & Inbound Pinhole Allowed: %d\n", firewallEnabled, inboundPinholeAllowed);
	LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "GetFirewallStatus:\n   Firewall Enabled: %s\n   Inbound Pinhole Allowed: %s\n", (firewallEnabled)? "Yes":"No", (inboundPinholeAllowed)? "Yes":"No");
					...
					const char * uniqueID, const char * lease_time)
{
	...
	if(isWorking || (r!=702 && r!=703 && r!=704))
	{
		...
		LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "UpdatePinhole: Pinhole ID = %s with Lease Time: %s\n", uniqueID, lease_time);
		if(r!=UPNPCOMMAND_SUCCESS)
			LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "UpdatePinhole: ID (%s) failed with code %d (%s)\n", uniqueID, r, strupnperror(r));
	}
}
...
#else
int runUpnp(int argc, char ** argv)
#endif // 0 // not needed for No Limit Connect
{
	char command = 0;
	char ** commandargv = 0;
	int commandargc = 0;
...
//#endif
	...
	for(i=1; i<argc; i++)
	{
		...
		if(argv[i][0] == '-')
		{
			...
			else if(argv[i][1] == 'z')
			{
				...
				{
					LogCModule( MODULE_PORT_FORWARD, LOG_ERROR, "Invalid localport '%s'\n", argv[i]);
		...
		{
			LogCModule( MODULE_PORT_FORWARD, LOG_ERROR, "option '%s' invalid\n", argv[i]);
		}
	}
	                             ...
	                             localport, ipv6, ttl, &error)))
	{
		...
		if(devlist)
		{
			LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "List of UPNP devices found on the network :\n");
			for(device = devlist; device; device = device->pNext)
			{
				LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, " desc: %s\n st: %s\n\n",
					   device->descURL, device->st);
			}
		}
		else if(!rootdescurl)
		{
			LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "upnpDiscover() error code=%d\n", error);
		}
		i = 1;
		  ...
		  || (i = UPNP_GetValidIGD(devlist, &urls, &data, lanaddr, sizeof(lanaddr))))
		{
			...
			#if 0
			LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "getting \"%s\"\n", urls.ipcondescURL);
			...
			case 'N':
				if (commandargc < 3)
					LogCModule( MODULE_PORT_FORWARD, LOG_ERROR, "too few arguments\n");
			...
			case 'r':
				...
				while(i<commandargc)
				{
					if(!is_int(commandargv[i])) {
						...
						LogCModule( MODULE_PORT_FORWARD, LOG_ERROR, "command -r : %s is not an port number\n", commandargv[i]);
						retcode = 1;
						break;
			...
			case 'G':
				...
				break;
			case 'P':
				LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "Presentation URL found:\n");
				LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "            %s\n", data.presentationurl);
				break;
			default:
				LogCModule( MODULE_PORT_FORWARD, LOG_ERROR, "Unknown switch -%c\n", command);
				retcode = 1;
			}
