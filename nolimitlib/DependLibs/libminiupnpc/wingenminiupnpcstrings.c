...
int main(int argc, char * * argv) {
  ...
  GetVersionEx(&osvi);
	...
	LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "Windows %lu.%lu Build %lu %s\n",
	...
	LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "MiniUPnPc version %s\n", miniupnpcVersion);

	if(argc >= 3) {
		fin = fopen(argv[1], "r");
		if(!fin) {
			LogCModule( MODULE_PORT_FORWARD, LOG_ERROR, "Cannot open %s for reading.\n", argv[1]);
			return 1;
		}
		fout = fopen(argv[2], "w");
		if(!fout) {
