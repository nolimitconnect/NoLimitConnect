...
getHTTPResponse(SOCKET s, int * size, int * status_code)
{
	...
	{
#ifdef DEBUG
		LogCModule( MODULE_PORT_FORWARD, LOG_ERROR, "%s: Memory allocation error\n", "getHTTPResponse");
#endif /* DEBUG */
		*size = -1;
		return NULL;
	}
...
/*		if(WSAAddressToStringA((SOCKADDR *)&saddr, sizeof(saddr),
                            NULL, addr_str, (DWORD *)&addr_str_len))
		{
			...
			if(n != 0) {
#ifdef _WIN32
				LogCModule( MODULE_PORT_FORWARD, LOG_ERROR, "getnameinfo() failed : %d\n", n);
#else
				LogCModule( MODULE_PORT_FORWARD, LOG_ERROR, "getnameinfo() failed : %s\n", gai_strerror(n));
#endif
			}
#endif
		}
