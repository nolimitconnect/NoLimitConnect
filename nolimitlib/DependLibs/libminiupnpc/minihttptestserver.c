...
{
	(void)sig;
	/* LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "handle_signal_chld(%d)\n", sig); */
	++child_to_wait_for;
}
...
{
	(void)sig;
	/* LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "handle_signal_int(%d)\n", sig); */
	quit = 1;
}
...
void handle_http_connection(int c)
{
	...
	http_version[i] = '\0';
	LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "Method = %s, URI = %s, %s\n",
...
static int server(unsigned short port, const char * expected_file_name, int ipv6)
{
	...
	if(port == 0) {
		...
		LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "Listening on port %hu\n", port);
		fflush(stdout);
	}
	...
	if(expected_file_name) {
		...
		if(f) {
			...
			if(buffer == NULL) {
				LogCModule( MODULE_PORT_FORWARD, LOG_ERROR, "memory allocation error\n");
			} else {
				build_content(buffer, 16*1024);
				i = fwrite(buffer, 1, 16*1024, f);
				if(i != 16*1024) {
					LogCModule( MODULE_PORT_FORWARD, LOG_ERROR, "error writing to file %s : %dbytes written (out of %d)\n", expected_file_name, i, 16*1024);
				}
				free(buffer);
			}
			fclose(f);
		} else {
			LogCModule( MODULE_PORT_FORWARD, LOG_ERROR, "error opening file %s for writing\n", expected_file_name);
		}
	}
	...
	while(!child && !quit) {
		while(child_to_wait_for > 0) {
			...
			if(pid < 0) {
				...
				LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "child(%d) terminated with status %d\n", (int)pid, status);
			}
			--child_to_wait_for;
		}
		...
		LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "accept...\n");
		pid = fork();
		if(pid < 0) {
			perror("fork");
			return 1;
	...
	if(!child) {
		...
		LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "Bye...\n");
	}
	return 0;
}
