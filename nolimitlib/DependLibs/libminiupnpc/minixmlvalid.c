...
int evtlistcmp(struct eventlist * a, struct eventlist * b)
{
	...
	if(a->n != b->n)
	{
		LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "event number not matching : %d != %d\n", a->n, b->n);
		/*return 1;*/
	}
	for(i=0; i<a->n; i++)
	{
		...
		{
			LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "Found a difference : %d '%.*s' != %d '%.*s'\n",
...
void startelt(void * data, const char * p, int l)
{
	...
	/*LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "startelt : %.*s\n", l, p);*/
	evt->type = ELTSTART;
	evt->data = p;
	evt->len = l;
	evtlist->n++;
...
void endelt(void * data, const char * p, int l)
{
	...
	struct event * evt;
	evt = evtlist->events + evtlist->n;
	/*LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "endelt : %.*s\n", l, p);*/
	evt->type = ELTEND;
	evt->data = p;
	evt->len = l;
	evtlist->n++;
...
void chardata(void * data, const char * p, int l)
{
	...
	struct event * evt;
	evt = evtlist->events + evtlist->n;
	/*LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "chardata : '%.*s'\n", l, p);*/
	evt->type = CHARDATA;
	evt->data = p;
	evt->len = l;
	evtlist->n++;
...
int testxmlparser(const char * xml, int size)
{
	...
	parsexml(&parser);
	LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "%d events\n", evtlist.n);
	/* compare */
...
int main(int argc, char * * argv)
{
	...
	if(r)
		LogCModule( MODULE_PORT_FORWARD, LOG_DEBUG, "minixml validation test failed\n");
	return r;
}
