...
#define _CRT_RAND_S // crap for microsoft rand_s
...
{
    NlcUrl pathToUrl( strFileNameAndPath );
    return GetTitleFromPath( pathToUrl, bIsFolder );
}

//========================================================================
std::string CUtil::GetTitleFromPath( const NlcUrl& url, bool bIsFolder /* = false */ )
{
  ...
  if (!CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(CSettings::SETTING_FILELISTS_SHOWEXTENSIONS) && !bIsFolder)
    ...
    strFilename = NlcUrl::Decode( strFilename );
    return strFilename;
}
...
{
    return URIUtils::HasExtension( strFile, CServiceBroker::GetFileExtensionProvider().GetPictureExtensions() + "|.tbn|.dds" );
}

//========================================================================
std::string CUtil::GetSplashPath()
{
    std::array<std::string, 4> candidates{ { "special://home/media/Splash.png", "special://xbmc/media/Splash.png" , "special://home/media/Splash.jpg", "special://xbmc/media/Splash.jpg"} };
...
std::string CUtil::ResolveExecutablePath( bool dirOnly )
{
    ...
    if( dirOnly )
    {
        // remove process name so is just directory
  ...
  for (const auto &pItem : items)
    {
        ...
        if( StringUtils::StartsWithNoCase( strCandidate, videoName ) )
        {
            ...
            else
            {
                ...
                CLog::Log( LOGINFO, "%s: found associated file %s\n", __FUNCTION__, NlcUrl::GetRedacted( pItem->GetPath() ).c_str() );
            }
        }
        else
        {
                                          ...
                                          std::vector<std::string>& associatedFiles )
{
  ...
  CLog::LogF(LOGDEBUG, "Scanning archive %s", NlcUrl::GetRedacted(strArchivePath).c_str());
    ...
    {
        NlcUrl pathToUrl( strArchivePath );
        NlcUrl zipURL = URIUtils::CreateArchivePath( "zip", pathToUrl, "" );
    ...
    {
        NlcUrl pathToUrl( strArchivePath );
        NlcUrl rarURL = URIUtils::CreateArchivePath( "rar", pathToUrl, "" );
...
void CUtil::ScanForExternalSubtitles( const std::string& strMovie, std::vector<std::string>& vecSubtitles )
{
  ...
  {
        if (!CDirectory::Exists(customPath))
        {
  ...
  for (size_t i = 0; i < iSize; i++)
        ...
        if( URIUtils::HasExtension( vecSubtitles[ i ], ".smi" ) )
        {
          ...
          for (const auto &lang : TagConv.m_Langclass)
            ...
            std::string strDest = StringUtils::Format("special://temp/subtitle.%s.%zu.smi", lang.Name.c_str(), i);
                        ...
                        {
                            CLog::Log( LOGINFO, " cached subtitle %s->%s\n", NlcUrl::GetRedacted( vecSubtitles[ i ] ).c_str(), strDest.c_str() );
    ...
    CLog::Log( LOGDEBUG, "%s: END (total time: %i ms)", __FUNCTION__, ( int )( XbmcThreadsNoChrono::SystemClockMillis() - startTimer ) );
}
...
ExternalStreamInfo CUtil::GetExternalStreamDetailsFromFilename( const std::string& videoPath, const std::string& associatedFile )
{
    ...
    else if( URIUtils::GetExtension( associatedFile ) == ".sub" && URIUtils::IsInArchive( associatedFile ) )
    {
        ...
        CLog::Log( LOGDEBUG, "%s - skipping archived vobsub filename parsing: %s", __FUNCTION__, NlcUrl::GetRedacted( associatedFile ).c_str() );
        toParse.clear();
    }
    ...
    CLog::Log( LOGDEBUG, "%s - Language = '%s' / Name = '%s' / Flag = '%u' from %s",
               __FUNCTION__, info.language.c_str(), info.name.c_str(), info.flag, NlcUrl::GetRedacted( associatedFile ).c_str() );
    return info;
}
...
bool CUtil::FindVobSubPair( const std::vector<std::string>& vecSubtitles, const std::string& strIdxPath, std::string& strSubPath )
{
    ...
    for (const auto &subtitlePath : vecSubtitles)
      ...
      if (URIUtils::IsInArchive(subtitlePath))
        strSubDirectory = NlcUrl::Decode(strSubDirectory);
...
std::string CUtil::GetVobSubSubFromIdx( const std::string& vobSubIdx )
{
  ...
  for (const std::string& archType : archTypes)
    ...
    vobSub = URIUtils::CreateArchivePath(archType,
                                         NlcUrl(URIUtils::ReplaceExtension(vobSubIdx, std::string(".") + archType)),
                                              vobSubFilename ).Get();
        if( CFile::Exists( vobSub ) )
            return vobSub;
    }
