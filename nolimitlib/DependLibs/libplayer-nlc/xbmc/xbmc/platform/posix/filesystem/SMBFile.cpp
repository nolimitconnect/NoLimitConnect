...
bool CSMBFile::Open(const NlcUrl& url)
{
  ...
  if (m_fd == -1)
  {
    ...
    CLog::Log(LOGINFO, "SMBFile->Open: Unable to open file : '%s'\nunix_err:'%x' error : '%s'", NlcUrl::GetRedacted(strFileName).c_str(), errno, strerror(errno));
    return false;
  }
...
bool CSMBFile::OpenForWrite(const NlcUrl& url, bool bOverWrite)
{
  ...
  if (bOverWrite)
  {
    CLog::Log(LOGWARNING, "SMBFile::OpenForWrite() called with overwriting enabled! - %s", NlcUrl::GetRedacted(strFileName).c_str());
