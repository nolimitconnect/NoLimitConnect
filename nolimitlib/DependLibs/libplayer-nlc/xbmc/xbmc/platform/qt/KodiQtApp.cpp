...
void CXBMCApp::Announce( ANNOUNCEMENT::AnnouncementFlag flag, const std::string& sender, const std::string& message, const CVariant &data )
{
    ...
    {
        if( strcmp( message.c_str(), "OnPlay" ) == 0 || strcmp( message.c_str(), "OnResume" ) == 0 )
            OnPlayBackStarted();
        else if( strcmp( message.c_str(), "OnPause" ) == 0 )
            OnPlayBackPaused();
        else if( strcmp( message.c_str(), "OnStop" ) == 0 )
            OnPlayBackStopped();
        else if( strcmp( message.c_str(), "OnSeek" ) == 0 )
            UpdateSessionState();
        else if( strcmp( message.c_str(), "OnSpeedChanged" ) == 0 )
            UpdateSessionState();
    }
    else if( flag & Info )
    {
        if( strcmp( message.c_str(), "OnChanged" ) == 0 )
...
void CXBMCApp::onPause()
{
    ...
    auto appPlayer = g_application.GetComponent<CApplicationPlayer>();
    ...
    if( appPlayer->IsPlaying() )
    {
        if( appPlayer->HasVideo() )
        {
            if( !appPlayer->IsPaused() && !m_hasReqVisible )
                CServiceBroker::GetAppMessenger()->SendMsg( TMSG_GUI_ACTION, WINDOW_INVALID, -1, static_cast<void*>( new CAction( ACTION_PAUSE ) ) );
        }
    }
...
void CXBMCApp::onStop()
{
    ...
    if( ( m_playback_state & PLAYBACK_STATE_PLAYING ) && !m_hasReqVisible )
    {
        if( m_playback_state & PLAYBACK_STATE_CANNOT_PAUSE )
            CServiceBroker::GetAppMessenger()->SendMsg( TMSG_GUI_ACTION, WINDOW_INVALID, -1, static_cast<void*>( new CAction( ACTION_STOP ) ) );
...
bool CXBMCApp::XBMC_SetupDisplay()
{
    ...
    bool result;
    CServiceBroker::GetAppMessenger()->SendMsg( TMSG_DISPLAY_SETUP, -1, -1, static_cast<void*>( &result ) );
    return result;
}
...
bool CXBMCApp::XBMC_DestroyDisplay()
{
    ...
    bool result;
    CServiceBroker::GetAppMessenger()->SendMsg( TMSG_DISPLAY_DESTROY, -1, -1, static_cast<void*>( &result ) );
    return result;
}
...
//        CServiceBroker::GetAppMessenger()->PostMsg(TMSG_GUI_ACTIVATE_WINDOW, WINDOW_VIDEO_NAV, 0, nullptr, "", params);
//      }
...
//        params.push_back(targeturl.Get());
//        params.push_back("return");
//        CServiceBroker::GetAppMessenger()->PostMsg(TMSG_GUI_ACTIVATE_WINDOW, WINDOW_MUSIC_NAV, 0, nullptr, "", params);
//      }
//    }
//    else
//    {
...
//  {
//    if (m_playback_state & PLAYBACK_STATE_CANNOT_PAUSE)
//      CServiceBroker::GetAppMessenger()->PostMsg(TMSG_GUI_ACTION, WINDOW_INVALID, -1, static_cast<void*>(new CAction(ACTION_STOP)));
//    else if (m_playback_state & PLAYBACK_STATE_VIDEO)
//      CServiceBroker::GetAppMessenger()->PostMsg(TMSG_GUI_ACTION, WINDOW_INVALID, -1, static_cast<void*>(new CAction(ACTION_PAUSE)));
//  }
//}
...
//    CServiceBroker::GetAppMessenger()->PostMsg(TMSG_GUI_ACTION, WINDOW_INVALID, -1, static_cast<void*>(
