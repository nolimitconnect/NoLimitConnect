...
{
  const NlcUrl url(favouritesURL);
  ...
  m_exec = CExecString(NlcUrl::Decode(url.GetHostName()));
  if (m_exec.IsValid())
...
bool CFavouritesURL::Parse(CFavouritesURL::Action action, const std::vector<std::string>& params)
{
  ...
  {
    // get the add-on name
    const std::string plugin = pathIsAddonID ? m_target : NlcUrl(m_target).GetHostName();
