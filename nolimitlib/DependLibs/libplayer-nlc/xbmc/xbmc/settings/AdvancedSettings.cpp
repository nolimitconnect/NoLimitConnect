...
void CAdvancedSettings::ParseSettingsFile(const std::string &file)
{
  ...
  CLog::Log(LOGINFO, "Contents of %s are...\n%s", file.c_str(),
            std::regex_replace(printer.CStr(), redactRe, "$1USERNAME:PASSWORD@").c_str());
    ...
    if (pAdjustRefreshrate)
    {
      ...
      while (pRefreshOverride)
      {
        ...
        if (fpsCorrect && refreshCorrect)
          ...
          CLog::Log(LOGWARNING,
                    "Ignoring malformed refreshrate override, fpsmin:%3.2f fpsmax:%3.2f "
                    "refreshmin:%3.2f refreshmax:%3.2f",
      ...
      while (pRefreshFallback)
      {
        ...
        if (fallback.refreshmin > 0.0f && fallback.refreshmax >= fallback.refreshmin)
          ...
          CLog::Log(LOGWARNING,
                    "Ignoring malformed refreshrate fallback, fpsmin:%3.2f fpsmax:%3.2f "
                    "refreshmin:%3.2f refreshmax:%3.2f",
    ...
    if (pVideoLatency)
    {
      ...
      while (pRefreshVideoLatency)
      {
        ...
        if (videolatency.refreshmin > 0.0f && videolatency.refreshmax >= videolatency.refreshmin)
          ...
          CLog::Log(LOGWARNING,
                    "Ignoring malformed display latency <refresh> entry, min:%3.2f max:%3.2f",
  ...
  if (pPathSubstitution)
  {
    ...
    while (pSubstitute)
    {
      ...
      if (!strFrom.empty() && !strTo.empty())
      {
        ...
        CLog::Log(LOGDEBUG, "    From: [%s]", NlcUrl::GetRedacted(strFrom).c_str());
        CLog::Log(LOGDEBUG, "    To:   [%s]", NlcUrl::GetRedacted(strTo).c_str());
