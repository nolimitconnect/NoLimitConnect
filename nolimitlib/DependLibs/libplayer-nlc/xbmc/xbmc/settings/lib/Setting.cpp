...
bool CSetting::Deserialize(const TiXmlNode *node, bool update /* = false */)
{
  ...
  if (dependencies != nullptr)
  {
    ...
    while (dependencyNode != nullptr)
    {
      ...
      if (dependency.Deserialize(dependencyNode))
        ...
        s_logger->warn("error reading <%s> tag of \"%s\"", SETTING_XML_ELM_DEPENDENCY, m_id.c_str());
  ...
  if (control != nullptr)
  {
    ...
    {
      s_logger->error("error reading \"%s\" attribute of <control> tag of \"%s\"",
    ...
    {
      s_logger->error("error reading <%s> tag of \"%s\"", SETTING_XML_ELM_CONTROL, m_id.c_str());
      return false;
    }
  }
  else if (!update && m_level < SettingLevel::Internal && !IsReference())
  {
    s_logger->error("missing <%s> tag of \"%s\"", SETTING_XML_ELM_CONTROL, m_id.c_str());
    return false;
  }
    ...
    while (updateElem != nullptr)
    {
      ...
      if (settingUpdate.Deserialize(updateElem))
      {
        if (!m_updates.insert(settingUpdate).second)
          s_logger->warn("duplicate <%s> definition for \"%s\"", SETTING_XML_ELM_UPDATE, m_id.c_str());
      }
      else
        s_logger->warn("error reading <%s> tag of \"%s\"", SETTING_XML_ELM_UPDATE, m_id.c_str());
...
bool CSettingList::Deserialize(const TiXmlNode *node, bool update /* = false */)
{
  ...
  if (element == nullptr)
  {
    s_logger->warn("unable to read type of list setting of %s", m_id.c_str());
    return false;
  }
  ...
  if (constraints != nullptr)
  {
    ...
    {
      s_logger->warn("invalid <%s> (%d) and/or <%s> (%d) of %s", SETTING_XML_ELM_MINIMUM_ITEMS,
  ...
  if (XMLUtils::GetString(node, SETTING_XML_ELM_DEFAULT, values))
  {
    ...
    {
      s_logger->warn("invalid <%s> definition \"%s\" of %s", SETTING_XML_ELM_DEFAULT, values.c_str(), m_id.c_str());
      return false;
    }
    Reset();
  }
...
bool CSettingBool::Deserialize(const TiXmlNode *node, bool update /* = false */)
{
  ...
  {
    s_logger->error("error reading the default value of \"%s\"", m_id.c_str());
    return false;
  }
  ...
  if (constraints != nullptr)
  {
    ...
    if (options != nullptr && options->FirstChild() != nullptr)
    {
      if (!DeserializeOptionsSort(options, m_optionsSort))
        s_logger->warn("invalid \"sort\" attribute of <" SETTING_XML_ELM_OPTIONS "> for \"%s\"",
                       m_id.c_str());
...
IntegerSettingOptions CSettingInt::UpdateDynamicOptions()
{
  ...
  if (m_optionsFiller == nullptr)
  {
    ...
    if (m_optionsFiller == nullptr)
    {
      s_logger->warn("unknown options filler \"%s\" of \"%s\"", m_optionsFillerName.c_str(), m_id.c_str());
      return options;
    }
  }
...
StringSettingOptions CSettingString::UpdateDynamicOptions()
{
  ...
  if (m_optionsFiller == nullptr)
  {
    ...
    if (m_optionsFiller == nullptr)
    {
      s_logger->error("unknown options filler \"%s\" of \"%s\"", m_optionsFillerName.c_str(), m_id.c_str());
      return options;
    }
  }
