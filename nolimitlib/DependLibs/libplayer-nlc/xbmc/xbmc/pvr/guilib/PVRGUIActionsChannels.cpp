...
bool CPVRGUIActionsChannels::StartChannelScan(int clientId)
{
  ...
  CLog::LogFC(LOGDEBUG, LOGPVR, "Starting to scan for channels on client %s",
              scanClient->GetFriendlyName().c_str());
  ...
  CLog::LogFC(LOGDEBUG, LOGPVR, "Channel scan finished after %3.2f ms", duration.count());
