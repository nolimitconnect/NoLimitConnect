...
bool CPVREpg::UpdateFromScraper(time_t start, time_t end, bool bForceUpdate)
{
  ...
  else if (m_strScraperName == "client")
  {
    ...
    if (client)
    {
      ...
      {
        CLog::LogF(LOGERROR, "The backend for channel '%s' on client '%d' does not support EPGs",
      ...
      {
        CLog::LogFC(LOGDEBUG, LOGEPG, "Updating EPG for channel '%s' from client '%s'",
                    m_channelData->ChannelName().c_str(), m_channelData->ClientId());
    ...
    {
      CLog::LogF(LOGERROR, "Client '%d' not found, can't update", m_channelData->ClientId());
    }
  }
