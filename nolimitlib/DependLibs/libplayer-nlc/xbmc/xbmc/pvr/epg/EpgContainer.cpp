...
bool CPVREpgContainer::PersistAll(unsigned int iMaxTimeslice) const
{
  ...
  if (!changedEpgs.empty())
  {
    ...
    for (const auto& epg : changedEpgs)
    {
      ...
      {
        CLog::LogFC(LOGDEBUG, LOGEPG, "EPG Container: Persisting events for channel '%s'...",
        ...
        {
          CLog::LogFC(LOGDEBUG, LOGEPG, "EPG Container: committing %d queries in loop.",
                      queryCount);
          database->CommitDeleteQueries();
          database->CommitInsertQueries();
          CLog::LogFC(LOGDEBUG, LOGEPG, "EPG Container: committed %d queries in loop.", queryCount);
        }
      }
