                ...
                const std::shared_ptr<CPVREpgInfoTag>& currentTag)
{
  ...
  if (previousTag->EndAsUTC() >= currentTag->EndAsUTC())
  {
    ...
    CLog::LogF(LOGDEBUG,
               ...
               previousTag->StartAsUTC().GetAsDBDateTime().c_str(),
               previousTag->EndAsUTC().GetAsDBDateTime().c_str(), currentTag->UniqueBroadcastID(),
               currentTag->Title().c_str(), currentTag->StartAsUTC().GetAsDBDateTime().c_str(),
               currentTag->EndAsUTC().GetAsDBDateTime().c_str());
...
void CPVREpgTagsContainer::QueuePersistQuery()
{
  if (m_database)
  {
    ...
    CLog::LogFC(LOGDEBUG, LOGEPG, "EPG Tags Container: Updating %d, deleting %d events...",
