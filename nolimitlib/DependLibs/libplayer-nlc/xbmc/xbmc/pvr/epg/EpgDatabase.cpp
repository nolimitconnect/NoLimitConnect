  ...
  if (ResultQuery(strQuery))
  {
    ...
    catch (...)
    {
      CLog::LogF(LOGERROR, "Could not load EPG tag with start time (%s) from the database",
                 startTime.GetAsDBDateTime().c_str());
    }
  }
    ...
    int iEpgID, const CDateTime& minStartTime)
{
  ...
  if (ResultQuery(strQuery))
  {
    ...
    {
      CLog::LogF(LOGERROR, "Could not load tags with min start time (%s) for EPG (%d)",
                 minStartTime.GetAsDBDateTime().c_str(), iEpgID);
    }
  }
                                                                       ...
                                                                       const CDateTime& maxEndTime)
{
  ...
  if (ResultQuery(strQuery))
  {
    ...
    {
      CLog::LogF(LOGERROR, "Could not load tags with max end time (%s) for EPG (%d)",
                 maxEndTime.GetAsDBDateTime().c_str(), iEpgID);
    }
  }
    ...
    int iEpgID, const CDateTime& minStartTime, const CDateTime& maxEndTime)
{
  ...
  if (ResultQuery(strQuery))
  {
    ...
    {
      CLog::LogF(LOGERROR,
                 "Could not load tags with min start time (%s) and max end time (%s) for EPG (%d)",
                 minStartTime.GetAsDBDateTime().c_str(), maxEndTime.GetAsDBDateTime().c_str(), iEpgID);
    }
  }
    ...
    int iEpgID, const CDateTime& minEndTime, const CDateTime& maxStartTime)
{
  ...
  if (ResultQuery(strQuery))
  {
    ...
    {
      CLog::LogF(LOGERROR,
                 "Could not load tags with min end time (%s) and max start time (%s) for EPG (%d)",
                 minEndTime.GetAsDBDateTime().c_str(), maxStartTime.GetAsDBDateTime().c_str(), iEpgID);
    }
  }
...
bool CPVREpgDatabase::Delete(const CPVREpgSearchFilter& epgSearch)
{
  ...
  CLog::LogFC(LOGDEBUG, LOGEPG, "Deleting saved search '%s' from the database",
              epgSearch.GetTitle().c_str());
