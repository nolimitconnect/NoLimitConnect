...
{
  CLog::LogFC(LOGDEBUG, LOGPVR, "Deleting provider '%s' from the database",
  ...
  if (ResultQuery(strQuery))
  {
    try
    {
      while (!m_pDS->eof())
      {
        ...
        CLog::LogFC(LOGDEBUG, LOGPVR, "Channel Provider '%s' loaded from PVR database",
                    provider->GetName().c_str());
        m_pDS->next();
      }
...
bool CPVRDatabase::QueueDeleteQuery(const CPVRChannel& channel)
{
  ...
  {
    CLog::LogF(LOGERROR, "Invalid channel id: %d", channel.ChannelID());
    return false;
  }

  CLog::LogFC(LOGDEBUG, LOGPVR, "Queueing delete for channel '%s' from the database",
...
{
  CLog::LogFC(LOGDEBUG, LOGPVR, "Queueing delete for channel group member '%s' from the database",
              groupMember.Channel() ? groupMember.Channel()->ChannelName().c_str()
                                    : std::to_string(groupMember.ChannelDatabaseID()).c_str());
  ...
  if (ResultQuery(query))
  {
    try
    {
      while (!m_pDS->eof())
      {
        ...
        CLog::LogFC(LOGDEBUG, LOGPVR, "Group '%s' loaded from PVR database", group->GroupName().c_str());
        m_pDS->next();
        iLoaded++;
      }
      m_pDS->close();
