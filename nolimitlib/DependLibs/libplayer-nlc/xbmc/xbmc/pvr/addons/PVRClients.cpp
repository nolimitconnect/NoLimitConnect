...
void LogClientWarning(const char* strFunctionName, const std::shared_ptr<CPVRClient>& client)
{
  if (client->IgnoreClient())
    CLog::Log(LOGWARNING, "%s: Not calling add-on '%s'. Add-on not (yet) connected.",
              strFunctionName, client->ID().c_str());
  else if (!client->ReadyToUse())
    CLog::Log(LOGWARNING, "%s: Not calling add-on '%s'. Add-on not ready to use.", strFunctionName,
              client->ID().c_str());
  else
    CLog::Log(LOGERROR, "%s: Not calling add-on '%s' for unexpected reason.", strFunctionName,
              client->ID().c_str());
}
