...
ADDON_STATUS CPVRClient::Create()
{
  ...
  CLog::LogFC(LOGDEBUG, LOGPVR, "Creating PVR add-on instance [%s,%d,%d]", ID().c_str(), InstanceId(),
              m_iClientId);
...
/*
  ...
  else
  {
    ...
    CLog::LogF(LOGERROR, "Failed to create PVR add-on instance %d. status=%d", GetID(), status);
  }
...
PVR_ERROR CPVRClient::OpenLiveStream(const std::shared_ptr<CPVRChannel>& channel)
{
  ...
  return DoAddonCall(__func__, [this, channel](const AddonInstance* addon) {
    ...
    if (!CanPlayChannel(channel))
    {
      CLog::LogFC(LOGDEBUG, LOGPVR, "Add-on %d can not play channel '%s'", GetID(),
                                     ...
                                     bool bForceCall /* = false */)
{
  ...
  {
    CLog::Log(LOGWARNING, LOGPVR, "%s: Ignoring callback from PVR client '%s'", strFunctionName,
              client->ID().c_str());
    return;
  }
