  ...
  for (const std::string& segment : segments)
  {
    switch (m_kind)
    {
      ...
      case Kind::ROOT:
      {
        ...
        {
          m_groupName = NlcUrl::Decode(tokens[0]);
        ...
        CLog::LogF(LOGERROR, "Invalid channels path '%s' - channel group segment syntax error.",
                   strPath.c_str());
        m_kind = Kind::INVALID;
        break;
      }
  ...
  : m_bRadio(bRadio), m_groupName(bHidden ? ".hidden" : strGroupName)
{
  ...
  else
  {
    ...
    m_path = StringUtils::Format("pvr://channels/{}/{}@{}/", bRadio ? "radio" : "tv",
                                 NlcUrl::Encode(m_groupName), m_groupClientID);
  }
}
  ...
  : m_bRadio(bRadio)
{
  if (!strGroupName.empty() && iGroupClientID >= -1 && !strAddonID.empty() && iChannelUID >= 0)
  {
    ...
    m_path = StringUtils::Format("pvr://channels/{}/{}@{}/{}@{}_{}.pvr", bRadio ? "radio" : "tv",
                                 NlcUrl::Encode(m_groupName), m_groupClientID, m_instanceID,
