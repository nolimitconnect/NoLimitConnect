...
{
  CLog::LogFC(LOGDEBUG, LOGPVR, "Got channel group '%s' from client %d.", group->GroupName().c_str(),
              group->GetClientID());
                                          ...
                                          bool bChannelsOnly /* = false */)
{
  ...
  for (const auto& group : groups)
  {
    if (!bChannelsOnly || group->IsInternalGroup())
    {
      ...
      {
        CLog::LogFC(LOGERROR, LOGPVR, "Failed to update channel group '%s'", group->GroupName().c_str());
        bReturn = false;
      }
    }
...
bool CPVRChannelGroups::LoadFromDatabase(const std::vector<std::shared_ptr<CPVRClient>>& clients)
{
  ...
  CLog::LogFC(LOGDEBUG, LOGPVR, "Fetched %d %s groups from the database", iLoaded,
              m_bRadio ? "radio" : "TV");
  ...
  for (const auto& group : m_groups)
  {
    if (!group->LoadFromDatabase(channels, clients))
    {
      CLog::LogFC(LOGERROR, LOGPVR,
                  "Failed to load members of %s channel group '%s' from the database",
                  m_bRadio ? "radio" : "TV", group->GroupName().c_str());
    }
  }
