    ...
    const std::vector<std::shared_ptr<CPVRClient>>& clients)
{
  ...
  CLog::LogFC(LOGDEBUG, LOGPVR, "Fetched %d %s group members from the database for group '%s'",
              iChannelCount, IsRadio() ? "radio" : "TV", GroupName().c_str());
    ...
    for (const auto& member : results)
    {
          ...
          GetClientID() != member->ChannelClientID())
      {
        CLog::LogF(LOGWARNING,
                   "Skipping member with channel database id %d of %s channel group '%s'. "
                   "Channel client id does not match group client id.",
                   member->ChannelDatabaseID(), IsRadio() ? "radio" : "TV", GroupName().c_str());
...
bool CPVRChannelGroup::UpdateFromClient(const std::shared_ptr<CPVRChannelGroupMember>& groupMember)
{
  ...
  if (existingMember)
  {
    ...
    {
      CLog::LogFC(LOGDEBUG, LOGPVR, "Updated %s channel '%s' from PVR client",
                  IsRadio() ? "radio" : "TV", channel->ChannelName().c_str());
      bChanged = true;
    }
    ...
    {
      CLog::LogFC(LOGDEBUG, LOGPVR, "Updated %s channel group member '%s' in group '%s'",
  ...
  else
  {
    ...
    CLog::LogFC(LOGDEBUG, LOGPVR, "Added %s channel group member '%s' to group '%s'",
                IsRadio() ? "radio" : "TV", channel->ChannelName().c_str(), GroupName().c_str());
    ...
    const std::vector<std::shared_ptr<CPVRChannelGroupMember>>& groupMembers)
{
  ...
  for (auto it = m_sortedMembers.begin(); it != m_sortedMembers.end();)
  {
    ...
    {
      CLog::LogF(LOGDEBUG,
                 "Removed %s channel '%s' from group '%s' because it has the wrong client id",
                 IsRadio() ? "radio" : "TV", channel->ChannelName().c_str(), GroupName().c_str());
    ...
    if (mapIt == membersMap.end())
    {
      ...
      {
        CLog::LogFC(LOGDEBUG, LOGPVR, "Removed stale %s channel '%s' from group '%s'. clientId=%d",
                    IsRadio() ? "radio" : "TV", channel->ChannelName().c_str(), GroupName().c_str(),
                    channel->ClientID());
        membersToRemove.emplace_back(*it);
...
bool CPVRChannelGroup::Persist()
{
  ...
  if (database)
  {
    CLog::LogFC(LOGDEBUG, LOGPVR, "Persisting channel group '%s' with %d channels", GroupName().c_str(),
...
void CPVRChannelGroup::OnSettingChanged()
{
  ...
  CLog::LogFC(LOGDEBUG, LOGPVR,
              "Renumbering channel group '%s' to use the backend channel order and/or numbers",
              GroupName().c_str());
