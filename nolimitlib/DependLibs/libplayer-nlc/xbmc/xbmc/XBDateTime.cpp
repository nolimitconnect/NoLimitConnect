...
std::string CDateTime::GetAsSaveString() const
{
  ...
  return StringUtils::Format("%04i%02i%02i_%02i%02i%02i", st.year, st.month, st.day,
                             st.hour, st.minute, st.second);
}
...
{
  std::string strOut;
  const std::string& strFormat = format.empty() ? g_langInfo.GetLanguageTimeFormat() : format;
...
{
  return GetAsLocalizedDate(g_langInfo.GetLanguageDateFormat(longDate));
}
...
{
  const std::string timeFormat = g_langInfo.GetLanguageTimeFormat();
