...
bool CApplication::Create()
{
  ...
  if (!caCert.empty())
  {
    if (XFILE::CFile::Exists(caCert))
    {
      ...
      CLog::Log(LOGDEBUG, "CApplication::Create - SSL_CERT_FILE: %s", caCert.c_str());
    }
    else
    {
      CLog::Log(LOGDEBUG, "CApplication::Create - Error reading SSL_CERT_FILE: %s -> ignored",
                caCert.c_str());
    }
  }
...
bool CApplication::CreateGUI()
{
  ...
  if (!m_pWinSystem)
  {
    CLog::Log(LOGFATAL, "CApplication::%s - unable to init windowing system", __FUNCTION__);
    CServiceBroker::UnregisterWinSystem();
    return false;
  }
...
bool CApplication::OnAction(const CAction &action)
{
  ...
  if( !m_pInertialScrollingHandler->CheckForInertialScrolling(&action) )
  {
    ...
    {
        LogMsg( LOG_VERBOSE, "%s CServiceBroker::GetGUI()->GetWindowManager().OnAction %d", __func__, action.GetID() );
