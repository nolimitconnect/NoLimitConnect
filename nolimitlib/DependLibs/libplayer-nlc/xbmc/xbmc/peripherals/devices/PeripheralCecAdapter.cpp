...
void CPeripheralCecAdapter::ProcessVolumeChange(void)
{
  ...
  CecVolumeChange pendingVolumeChange = VOLUME_CHANGE_NONE;
  {
    ...
    if (!m_volumeChangeQueue.empty())
    {
      ...
      bool bRefresh(XbmcThreadsNoChrono::SystemClockMillis() - m_lastKeypress > VOLUME_REFRESH_TIMEOUT);

      /* only send the keypress when it hasn't been sent yet */
      if (pendingVolumeChange != m_lastChange)
      {
        m_lastKeypress = XbmcThreadsNoChrono::SystemClockMillis();
    ...
    else if (m_lastKeypress > 0 && XbmcThreadsNoChrono::SystemClockMillis() - m_lastKeypress > VOLUME_CHANGE_TIMEOUT)
    {
      /* send a key release */
      m_lastKeypress = 0;
      bSendRelease = true;
...
bool CPeripheralCecAdapter::ReopenConnection(bool bAsync /* = false */)
{
  if (bAsync)
  {
    CServiceBroker::GetJobManager()->AddJob(new CPeripheralCecAdapterReopenJob(this), nullptr, CJob::PRIORITY_NORMAL);
    return true;
  }
