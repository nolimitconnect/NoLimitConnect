...
bool CImageLoader::DoWork()
{
  ...
  if (!loadPath.empty())
  {
    ...
    m_texture =
        CTextureBase::LoadFromFile(loadPath, CServiceBroker::GetWinSystem()->GetGfxContext().GetWidth(),
...
void CGUILargeTextureManager::CLargeTexture::SetTexture(std::unique_ptr<CTextureBase> texture)
{
