...
bool URIUtils::GetParentPath(const std::string& strPath, std::string& strParent)
{
  ...
  else if (url.IsProtocol("stack"))
  {
    ...
    if (!dir.GetDirectory(url, items))
      return false;
    NlcUrl url2(GetDirectory(items[0]->GetPath()));
...
bool URIUtils::IsInternetStream(const NlcUrl& url, bool bStrictCheck /* = false */)
{
  ...
  if (NlcUrl::IsProtocolEqual(protocol, "http")  || NlcUrl::IsProtocolEqual(protocol, "https")  ||
      ...
      NlcUrl::IsProtocolEqual(protocol, "rtmps") || NlcUrl::IsProtocolEqual(protocol, "shout") ||
      NlcUrl::IsProtocolEqual(protocol, "rss") || NlcUrl::IsProtocolEqual(protocol, "rsss"))
    return true;
