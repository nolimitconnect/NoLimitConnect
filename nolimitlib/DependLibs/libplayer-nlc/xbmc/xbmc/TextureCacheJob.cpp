...
bool CTextureCacheJob::CacheTexture(std::unique_ptr<CTextureBase>* out_texture)
{
  ...
  std::unique_ptr<CTextureBase> texture = LoadImage(image, width, height, additional_info, true);
  if (texture)
  {
    if (texture->HasAlpha())
...
std::unique_ptr<CTextureBase> CTextureCacheJob::LoadImage(const std::string& image,
                                                      unsigned int width,
                                                      unsigned int height,
  ...
  std::unique_ptr<CTextureBase> texture =
      CTextureBase::LoadFromFile(image, width, height, requirePixels, file.GetMimeType());
  if (!texture)
    return NULL;
