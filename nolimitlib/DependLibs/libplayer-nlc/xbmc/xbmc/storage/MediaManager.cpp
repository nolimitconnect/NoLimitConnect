...
std::string CMediaManager::GetDiskUniqueId(const std::string& devicePath)
{
  ...
  if (info.empty())
  {
    CLog::Log(LOGDEBUG, "GetDiskUniqueId: Retrieving ID for path %s failed, ID is empty.", NlcUrl::GetRedacted(mediaPath).c_str());
    return "";
  }

  std::string strID = StringUtils::Format("removable://%s_%s", info.name.c_str(), info.serial.c_str());
  CLog::Log(LOGDEBUG, "GetDiskUniqueId: Got ID %s for %s with path %s", strID.c_str(), info.type.c_str(), NlcUrl::GetRedacted(mediaPath).c_str());
...
#endif // defined(HAS_OPTICAL_DRIVE)
