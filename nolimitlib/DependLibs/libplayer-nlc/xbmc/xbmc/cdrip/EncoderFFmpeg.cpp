...
bool CEncoderFFmpeg::Init(AddonToKodiFuncTable_AudioEncoder& callbacks)
{
  ...
  const AVCodec *codec = avcodec_find_encoder(m_Format->oformat->audio_codec);
  ...
  // BRJ FIXME
  // BRJ m_CodecCtx                 = m_Stream->codec;
  m_CodecCtx->codec_id       = codec->id;
  m_CodecCtx->codec_type     = AVMEDIA_TYPE_AUDIO;
  m_CodecCtx->bit_rate       = m_Format->bit_rate;
  m_CodecCtx->sample_rate    = m_iInSampleRate;
  // BRJ m_CodecCtx->channels       = m_iInChannels;
  // BRJ m_CodecCtx->channel_layout = av_get_default_channel_layout(m_iInChannels);
  ...
  if(!m_BufferFrame || !m_Buffer)
  {
    ...
    int err = av_frame_get_buffer(m_BufferFrame, 0);
    if (err < 0)
  ...
  if(m_NeedConversion)
  {
    ...
    m_ResampledFrame->ch_layout = m_CodecCtx->ch_layout;
