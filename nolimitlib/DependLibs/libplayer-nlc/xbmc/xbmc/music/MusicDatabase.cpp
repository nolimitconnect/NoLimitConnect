                                 ...
                                 const std::string& strImage)
{
  ...
  {
    CLog::Log(LOGDEBUG, "%s: Updating %s (Id: %d) mbid %d already assigned to %s (Id: %d)",
              __FUNCTION__, strMusicBrainzArtistID.c_str(), artistname.c_str(), idArtistMbid, strArtist.c_str(), idArtist);
    useMBIDNull = true;
    isScrapedMBID = false;
  }
                                             ...
                                             const std::string& strMusicBrainzArtistID)
{
  ...
  if (idArtistMbid > 0 && idArtistMbid != idArtist)
  {
    CLog::Log(LOGDEBUG, "%s: Artist mbid %s already assigned to %s (Id: %d)", __FUNCTION__,
...
bool CMusicDatabase::GetArtist(int idArtist, CArtist& artist, bool fetchAll /* = false */)
{
  try
  {
    ...
    CLog::Log(LOGDEBUG, LOGDATABASE, "%s(%s) - took %3.2f ms", __FUNCTION__, strSQL.c_str(),
              duration.count());
...
bool CMusicDatabase::AddSongGenres(int idSong, const std::vector<std::string>& genres)
{
  ...
  {
    CLog::Log(LOGERROR, "%s(%d) %s failed", __FUNCTION__, idSong, strSQL.c_str());
  }
  return false;
}
...
bool CMusicDatabase::GetRecentlyPlayedAlbums(VECALBUMS& albums)
{
  try
  {
    ...
    CLog::Log(LOGDEBUG, "%s: Time to fill list with albums %3.2fms query took %3.2fms", __FUNCTION__,
...
bool CMusicDatabase::Search(const std::string& search, CFileItemList& items)
{
  ...
  auto end = std::chrono::steady_clock::now();
  auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
  CLog::Log(LOGDEBUG, "%s Artist search in %3.2f ms", __FUNCTION__, duration.count());
  ...
  CLog::Log(LOGDEBUG, "%s Album search in %3.2f ms", __FUNCTION__, duration.count());
  ...
  duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
  CLog::Log(LOGDEBUG, "%s Songs search in %3.2f ms", __FUNCTION__, duration.count());
...
int CMusicDatabase::Cleanup(CGUIDialogProgress* progressDialog /*= nullptr*/)
{
  ...
  CLog::Log(LOGINFO, "%s: Cleaning musicdatabase done. Operation took %3.2fs", __FUNCTION__,
            duration.count());
                                  ...
                                  bool countOnly /* = false */)
{
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      std::string strDir = StringUtils::Format("%d/", m_pDS->fv("genre.idGenre").get_asInt());
                                   ...
                                   bool countOnly /*= false*/)
{
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      CMusicDbUrl itemUrl = musicUrl;
      std::string strDir = StringUtils::Format("%d/", m_pDS->fv("source.idSource").get_asInt());
      itemUrl.AppendPath(strDir);
                                 ...
                                 const Filter& filter /* = Filter() */)
{
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      CMusicDbUrl itemUrl = musicUrl;
      std::string strDir = StringUtils::Format("%d/", m_pDS->fv(0).get_asInt());
      itemUrl.AppendPath(strDir);
      if (useOriginalYears)
                                 ...
                                 const Filter& filter /* = Filter() */)
{
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      CMusicDbUrl itemUrl = musicUrl;
      std::string strDir = StringUtils::Format("%d/", m_pDS->fv("role.idRole").get_asInt());
      itemUrl.AppendPath(strDir);
    ...
    bool countOnly /* = false */)
{
  ...
  try
  {
    ...
    for (const auto& i : results)
    {
      ...
      {
        m_pDS->close();
        CLog::Log(LOGERROR, "%s - out of memory getting listing (got %d)", __FUNCTION__,
                  items.Size());
      }
    }
    // cleanup
    ...
    CLog::Log(LOGDEBUG, "%s: Time to fill list with artists %3.2f ms query took %3.2f ms",
    ...
    bool countOnly /* = false */)
{
  ...
  try
  {
    ...
    CLog::Log(LOGDEBUG, "%s: Time to fill list with albums %3.2f ms query took %3.2f ms", __FUNCTION__,
              duration.count(), queryDuration.count());
    ...
    for (const auto& i : results)
    {
      ...
      try
      {
        ...
        { // Make (fake) disc title from disc number, group by disc number as no real title to match
          strDiscSubtitle = StringUtils::Format("%s %d", g_localizeStrings.Get(427).c_str(), discnum);
          useTitle = false;
        }
    ...
    bool artistData /* = false*/)
{
  ...
  try
  {
    ...
    CLog::Log(LOGDEBUG, "%s: Time to fill list with songs %3.2f ms query took %3.2f ms", __FUNCTION__,
              duration.count(), queryDuration.count());
    ...
    const SortDescription& sortDescription /* = SortDescription() */)
{
  ...
  try
  {
    ...
    CLog::Log(LOGDEBUG, "%s - query took %3.2f ms", __FUNCTION__, duration.count());
...
bool CMusicDatabase::SetSongUserrating(const std::string& filePath, int userrating)
{
  ...
  {
    CLog::Log(LOGERROR, "%s (%s,%d) failed", __FUNCTION__, filePath.c_str(), userrating);
  }
  return false;
}
...
bool CMusicDatabase::SetSongUserrating(int idSong, int userrating)
{
  ...
  {
    CLog::Log(LOGERROR, "%s (%d,%d) failed", __FUNCTION__, idSong, userrating);
  }
  return false;
}
...
bool CMusicDatabase::SetAlbumUserrating(const int idAlbum, int userrating)
{
  ...
  {
    CLog::Log(LOGERROR, "%s (%d,%d) failed", __FUNCTION__, idAlbum, userrating);
  }
  return false;
}
...
bool CMusicDatabase::SetSongVotes(const std::string& filePath, int votes)
{
  ...
  {
    CLog::Log(LOGERROR, "%s (%s,%d) failed", __FUNCTION__, filePath.c_str(), votes);
  }
  return false;
}
                                ...
                                const ADDON::ScraperPtr& scraper)
{
  ...
  {
    RollbackTransaction();
    CLog::Log(LOGERROR, "%s - (%d, %s) failed", __FUNCTION__, id, strSQL.c_str());
  }
  return false;
}
...
bool CMusicDatabase::GetScraper(int id, const CONTENT_TYPE& content, ADDON::ScraperPtr& scraper)
{
  ...
  {
    CLog::Log(LOGERROR, "%s -(%d, %s %s) failed", __FUNCTION__, id, scraperUUID.c_str(), strSettings.c_str());
  }
  return false;
}
...
bool CMusicDatabase::ExportSongHistory(TiXmlNode* pNode, CGUIDialogProgress* progressDialog)
{
  try
  {
    ...
    CLog::Log(LOGDEBUG, "%s - %s", __FUNCTION__, strSQL.c_str());
    m_pDS->query(strSQL);
                                       ...
                                       CGUIDialogProgress* progressDialog)
{
  ...
  try
  {
    ...
    CLog::Log(LOGINFO, "%s: Create temporary HistSong table and insert %d records", __FUNCTION__,
              total);
...
void CMusicDatabase::SetItemUpdated(int mediaId, const std::string& mediaType)
{
  ...
  {
    CLog::Log(LOGERROR, "CMusicDatabase::%s (%s, %s) - failed to execute %s", __FUNCTION__,
