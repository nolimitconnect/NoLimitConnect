		...
		while( 1 )
		{
			...
			if( XFILE::CFile::Stat( archiveName, &s ) == 0 )
			{
...
#endif
				{
					...
					if( is_apk )
					{
						NlcUrl c( "apk://" + archiveName + "/" + strURL.substr( iPos + 1 ) );
						*this = c;
					}
					else
					{
						NlcUrl c( "zip://" + archiveName + "/" + strURL.substr( iPos + 1 ) );
						*this = c;
					}
					return;
				}
...
void NlcUrl::SetFileName( const std::string& strFileName )
{
...
void NlcUrl::SetProtocol( const std::string& strProtocol )
{
	m_strProtocol = strProtocol;
	StringUtils::ToLower( m_strProtocol );
}

void NlcUrl::SetOptions( const std::string& strOptions )
{
	m_strOptions.clear();
	m_options.Clear();
...
void NlcUrl::SetProtocolOptions( const std::string& strOptions )
{
	m_strProtocolOptions.clear();
	m_protocolOptions.Clear();
	if( strOptions.length() > 0 )
...
const std::string NlcUrl::GetTranslatedProtocol() const
{
	if( IsProtocol( "shout" )
		|| IsProtocol( "dav" )
		|| IsProtocol( "rss" ) )
...
const std::string NlcUrl::GetFileNameWithoutPath() const
{
...
char NlcUrl::GetDirectorySeparator() const
{
#ifndef TARGET_POSIX
...
std::string NlcUrl::GetWithoutOptions() const
{
	if( m_strProtocol.empty() )
		return m_strFileName;
...
std::string NlcUrl::GetWithoutUserDetails( bool redact ) const
{
	std::string strURL;
  ...
  if (redact && !m_strUserName.empty())
  {
	...
	if( !m_strHostName.empty() )
	{
		...
		if( URIUtils::HasParentInHostname( *this ) )
			strHostName = NlcUrl( m_strHostName ).GetWithoutUserDetails();
		else
...
std::string NlcUrl::GetWithoutFilename() const
{
	if( m_strProtocol.empty() )
		return "";
...
std::string NlcUrl::GetRedacted() const
{
...
std::string NlcUrl::Decode( const std::string& strURLData )
...
std::string NlcUrl::Encode( const std::string& strURLData )
{
	std::string strResult;
...
bool NlcUrl::IsProtocolEqual( const std::string &protocol, const char *type )
{
	/*
	 ...
	 We possibly shouldn't do this (as NlcUrl(foo).Get() != foo, though there are other reasons for this as well)
...
void NlcUrl::GetOptions( std::map<std::string, std::string> &options ) const
{
...
bool NlcUrl::HasOption( const std::string &key ) const
{
	return m_options.HasOption( key );
}

bool NlcUrl::GetOption( const std::string &key, std::string &value ) const
{
	CVariant valueObj;
...
std::string NlcUrl::GetOption( const std::string &key ) const
{
	std::string value;
...
void NlcUrl::SetOption( const std::string &key, const std::string &value )
{
	m_options.AddOption( key, value );
	SetOptions( m_options.GetOptionsString( true ) );
}

void NlcUrl::RemoveOption( const std::string &key )
{
	m_options.RemoveOption( key );
	SetOptions( m_options.GetOptionsString( true ) );
}

void NlcUrl::GetProtocolOptions( std::map<std::string, std::string> &options ) const
{
...
bool NlcUrl::HasProtocolOption( const std::string &key ) const
{
	return m_protocolOptions.HasOption( key );
}

bool NlcUrl::GetProtocolOption( const std::string &key, std::string &value ) const
{
	CVariant valueObj;
...
std::string NlcUrl::GetProtocolOption( const std::string &key ) const
{
	std::string value;
...
void NlcUrl::SetProtocolOption( const std::string &key, const std::string &value )
{
	m_protocolOptions.AddOption( key, value );
	m_strProtocolOptions = m_protocolOptions.GetOptionsString( false );
}

void NlcUrl::RemoveProtocolOption( const std::string &key )
{
