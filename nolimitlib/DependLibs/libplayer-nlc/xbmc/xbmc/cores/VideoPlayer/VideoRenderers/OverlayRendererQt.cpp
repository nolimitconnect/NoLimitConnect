...
#else
    nlc.glFuncPixelStorei( GL_UNPACK_ROW_LENGTH, stride / bytesPerPixel );
#endif

    nlc.glFuncPixelStorei( GL_UNPACK_ALIGNMENT, 1 );

    nlc.glFuncTexImage2D( target, 0, internalFormat
                  , width2, height2, 0
                  , externalFormat, GL_UNSIGNED_BYTE, NULL );

    nlc.glFuncTexSubImage2D( target, 0
                     , 0, 0, width, height
...
#ifndef HAS_GLES
    nlc.glFuncPixelStorei( GL_UNPACK_ROW_LENGTH, 0 );
#endif
...
COverlayTextureQt::COverlayTextureQt( const CDVDOverlayImage& o, CRect& rSource )
{
    ...
    nlc.glFuncGenTextures( 1, &m_texture );
    nlc.glFuncBindTexture( GL_TEXTURE_2D, m_texture );

    nlc.glFuncTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP );
    nlc.glFuncTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP );
    nlc.glFuncTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
    nlc.glFuncTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR );
  ...
  nlc.glFuncBindTexture(GL_TEXTURE_2D, 0);
...
void COverlayGlyphQt::Render( SRenderState& state )
{
    ...
    nlc.glFuncEnable( GL_BLEND );

    nlc.glFuncBindTexture( GL_TEXTURE_2D, m_texture );
    nlc.glFuncBlendFunc( GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA );

    nlc.glFuncTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE );
    nlc.glFuncTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE );
    nlc.glFuncTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
    nlc.glFuncTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR );
    ...
    nlc.glFuncVertexAttribPointer( posLoc, 3, GL_FLOAT, GL_FALSE, sizeof( VERTEX ), ( char* )vertices + offsetof( VERTEX, x ) );
    nlc.glFuncVertexAttribPointer( colLoc, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof( VERTEX ), ( char* )vertices + offsetof( VERTEX, r ) );
    nlc.glFuncVertexAttribPointer( tex0Loc, 2, GL_FLOAT, GL_FALSE, sizeof( VERTEX ), ( char* )vertices + offsetof( VERTEX, u ) );

    nlc.glFuncEnableVertexAttribArray( posLoc );
    nlc.glFuncEnableVertexAttribArray( colLoc );
    nlc.glFuncEnableVertexAttribArray( tex0Loc );

    nlc.glFuncDrawArrays( GL_TRIANGLES, 0, vecVertices.size() );

    nlc.glFuncDisableVertexAttribArray( posLoc );
    nlc.glFuncDisableVertexAttribArray( colLoc );
    nlc.glFuncDisableVertexAttribArray( tex0Loc );
