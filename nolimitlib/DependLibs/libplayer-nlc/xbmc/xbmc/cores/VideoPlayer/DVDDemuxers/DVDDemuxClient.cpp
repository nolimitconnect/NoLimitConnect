...
bool CDVDDemuxClient::ParsePacket(DemuxPacket* pkt)
{
  ...
  if (len >= 0)
  {
    ...
    {
     LogModule( eLogPlayerNlc, LOG_DEBUG, "CDVDDemuxClient::ParsePacket - (%d) profile changed from %d to %d", st->uniqueId, st->profile, stream->m_context->profile);
    ...
    {
      LogModule( eLogPlayerNlc, LOG_DEBUG, "CDVDDemuxClient::ParsePacket - (%d) level changed from %d to %d", st->uniqueId, st->level, stream->m_context->level);
    ...
    switch (st->type)
    {
      case STREAM_AUDIO:
      {
        ...
        {
          LogModule( eLogPlayerNlc, LOG_DEBUG, "CDVDDemuxClient::ParsePacket - (%d) channels changed from %d to %d",
        ...
        {
          LogModule( eLogPlayerNlc, LOG_DEBUG, "CDVDDemuxClient::ParsePacket - (%d) samplerate changed from %d to %d", st->uniqueId, sta->iSampleRate, stream->m_context->sample_rate);
      ...
      case STREAM_VIDEO:
      {
        ...
        {
          LogModule( eLogPlayerNlc, LOG_DEBUG, "CDVDDemuxClient::ParsePacket - (%d) width changed from %d to %d",
        ...
        {
          LogModule( eLogPlayerNlc, LOG_DEBUG, "CDVDDemuxClient::ParsePacket - (%d) height changed from %d to %d",
        ...
        if (stream->m_context->sample_aspect_ratio.num && stream->m_context->height)
        {
          ...
          {
            LogModule( eLogPlayerNlc, LOG_DEBUG, "CDVDDemuxClient::ParsePacket - (%d) aspect changed from %3.3f to %3.3f",
        ...
        {
          LogModule( eLogPlayerNlc, LOG_DEBUG, "CDVDDemuxClient::ParsePacket - (%d) fps changed from %d/%d to %d/%d",
...
void CDVDDemuxClient::SetStreamProps(CDemuxStream *stream, std::map<int, std::shared_ptr<CDemuxStream>> &map, bool forceInit)
{
  ...
  LogModule( eLogPlayerNlc, LOG_DEBUG, "CDVDDemuxClient::RequestStream(): added/updated stream %d with codec_id %d",
