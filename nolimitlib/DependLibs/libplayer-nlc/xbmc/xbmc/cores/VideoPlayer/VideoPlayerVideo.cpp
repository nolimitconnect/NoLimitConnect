...
bool CVideoPlayerVideo::OpenStream( CDVDStreamInfo hint )
{
    ...
    // clang-format off
        ...
        {
            LogModule( eLogPlayerNlc, LOG_ERROR, "Codec id %d require extradata.", hint.codec );
            return false;
        }
        // clang-format on
    }
...
void CVideoPlayerVideo::CloseStream( bool bWaitForBuffers )
{
    ...
    LogModule( eLogPlayerNlc, LOG_DEBUG, "waiting for video thread to exit" );
    ...
    LogModule( eLogPlayerNlc, LOG_DEBUG, "deleting video codec" );
    m_pVideoCodec.reset();
...
{
    CLog::Log( LOGNOTICE, "running thread: video_thread %d", VxGetCurrentThreadId() );
    ...
    while( !m_bStop )
    {
        ...
        {
            LogModule( eLogVideoIo, LOG_DEBUG, " CVideoPlayerVideo::Process msg GENERAL_SYNCHRONIZE" );
        ...
        else if( pMsg->IsType( CDVDMsg::GENERAL_RESYNC ) )
        {
            ...
            LogModule( eLogVideoIo, LOG_DEBUG, " CVideoPlayerVideo::Process msg GENERAL_RESYNC pts %3.3f", pts );
        ...
        else if( pMsg->IsType( CDVDMsg::VIDEO_SET_ASPECT ) )
        {
            ...
            LogModule( eLogVideoIo, LOG_DEBUG, " CVideoPlayerVideo::Process msg VIDEO_SET_ASPECT aspect %3.3f", m_fForcedAspectRatio );
        }
        else if( pMsg->IsType( CDVDMsg::GENERAL_RESET ) )
        {
            LogModule( eLogVideoIo, LOG_DEBUG, " CVideoPlayerVideo::Process msg GENERAL_RESET" );
            if( m_pVideoCodec )
                m_pVideoCodec->Reset();
        ...
        {
            LogModule( eLogVideoIo, LOG_DEBUG, " CVideoPlayerVideo::Process msg GENERAL_FLUSH" );
        ...
        else if( pMsg->IsType( CDVDMsg::PLAYER_SETSPEED ) )
        {
            ...
            LogModule( eLogVideoIo, LOG_DEBUG, " CVideoPlayerVideo::Process msg PLAYER_SETSPEED %d", m_speed );
            if( m_pVideoCodec )
        ...
        else if( pMsg->IsType( CDVDMsg::GENERAL_STREAMCHANGE ) )
        {
            ...
            LogModule( eLogVideoIo, LOG_DEBUG, " CVideoPlayerVideo::Process msg GENERAL_STREAMCHANGE " );
        ...
        {
            LogModule( eLogVideoIo, LOG_DEBUG, " CVideoPlayerVideo::Process msg VIDEO_DRAIN " );
            while( !m_bStop && m_pVideoCodec )
            {
                m_pVideoCodec->SetCodecControl( DVD_CODEC_CTRL_DRAIN );
                if( !ProcessDecoderOutput( frametime, pts ) )
        ...
        else if( pMsg->IsType( CDVDMsg::PLAYER_REQUEST_STATE ) )
        {
            ...
            LogModule( eLogVideoIo, LOG_VERBOSE, " CVideoPlayerVideo::Process msg PLAYER_REQUEST_STATE sync state %d ", m_syncState );
            m_messageParent.Put(
        ...
        else if( pMsg->IsType( CDVDMsg::DEMUXER_PACKET ) )
        {
            ...
            {
                LogModule( eLogVideoIo, LOG_DEBUG, " CVideoPlayerVideo::Process msg DEMUXER_PACKET added vid pkt FAILED" );
    ...
    LogModule( eLogVideoIo, LOG_DEBUG, " CVideoPlayerVideo::Process Thread %d EXITING", VxGetCurrentThreadId() );
}
...
bool CVideoPlayerVideo::ProcessDecoderOutput( double& frametime, double& pts )
{
    ...
    static CDVDVideoCodec::VCReturn lastDecoderState = CDVDVideoCodec::VCReturn::VC_NONE;
    ...
    {
        LogModule( eLogVideoIo, LOG_VERBOSE, " CVideoPlayerVideo VC_REOPEN pkt %d ", m_PktCnt );
        while( !m_packets.empty() )
        {
            auto msg = std::static_pointer_cast<CDVDMsgDemuxerPacket>(m_packets.front().message);
            m_packets.pop_front();
    ...
    {
        LogModule( eLogVideoIo, LOG_VERBOSE, " CVideoPlayerVideo VC_EOF pkt %d ", m_PktCnt );
        if( m_syncState == IDVDStreamPlayer::SYNC_STARTING )
        {
            ...
            LogModule( eLogVideoIo, LOG_VERBOSE, " CVideoPlayerVideo PLAYER_STARTED time %3.3f cache %3.3f cache total %3.3f ",
                       msg.timestamp, msg.cachetime, msg.cachetotal );
    ...
    // if both dts/pts invalid, use pts calculated from picture.iDuration
        ...
        if( m_outputSate == OUTPUT_AGAIN )
        {
            LogModule( eLogVideoIo, LOG_VERBOSE, "CVideoPlayerVideo VC_PICTURE OUTPUT_AGAIN" );
            return true;
        }
        else if( m_outputSate == OUTPUT_ABORT )
        {
            LogModule( eLogVideoIo, LOG_VERBOSE, "CVideoPlayerVideo VC_PICTURE OUTPUT_ABORT" );
            return false;
        }
        else if( (m_outputSate == OUTPUT_DROPPED) && !(m_picture.iFlags & DVP_FLAG_DROPPED) )
        {
            m_iDroppedFrames++;
            m_ptsTracker.Flush();
            LogModule( eLogVideoIo, LOG_VERBOSE, "CVideoPlayerVideo VC_PICTURE OUTPUT_DROPPED" );
        }
            ...
            !(m_picture.iFlags & DVP_FLAG_DROPPED) )
        {
            ...
            LogModule( eLogVideoIo, LOG_VERBOSE, "CVideoPlayerVideo VC_PICTURE SYNC_WAITSYNC PLAYER_STARTED" );
            m_messageParent.Put( std::make_shared<CDVDMsgType<SStartMsg>>( CDVDMsg::PLAYER_STARTED, msg ) );
        }
...
CVideoPlayerVideo::EOutputState CVideoPlayerVideo::OutputPicture( const VideoPicture* pPicture )
{
    ...
    if( buffer < 0 )
    {
        if( m_speed != DVD_PLAYSPEED_PAUSE )
            CLog::Log( LOGWARNING, "%s - timeout waiting for buffer", __FUNCTION__ );
        return OUTPUT_AGAIN;
    }
...
int CVideoPlayerVideo::CalcDropRequirement( double pts )
{
    ...
    if( m_bAllowDrop )
    {
        if( iSkippedPicture > 0 )
        {
            ...
            result |= DROP_DROPPED;
            CLog::Log( LOGDEBUG, LOGVIDEO, "CVideoPlayerVideo::CalcDropRequirement - dropped pictures, lateframes: %d, Bufferlevel: %d, dropped: %d\n", lateframes, iBufferLevel, iSkippedPicture );
        }
        if( iDroppedFrames > 0 )
        {
            CDroppingStats::CGain gain;
            ...
            result |= DROP_DROPPED;
            CLog::Log( LOGDEBUG, LOGVIDEO, "CVideoPlayerVideo::CalcDropRequirement - dropped in decoder, lateframes: %d, Bufferlevel: %d, dropped: %d\v", lateframes, iBufferLevel, iDroppedFrames );
        }
    }
