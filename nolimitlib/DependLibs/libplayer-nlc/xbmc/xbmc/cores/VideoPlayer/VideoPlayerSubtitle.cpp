...
bool CVideoPlayerSubtitle::OpenStream(CDVDStreamInfo &hints, std::string &filename)
{
  ...
  if(filename.size() && filename != "dvd" )
  {
    ...
    CLog::Log(LOGDEBUG, "Created subtitles parser: %s", m_pSubtitleFileParser->GetName().c_str());
  ...
  if(m_pOverlayCodec)
  {
    CLog::Log(LOGDEBUG, "Created subtitles overlay codec: %s", m_pOverlayCodec->GetName().c_str());
    return true;
  }
