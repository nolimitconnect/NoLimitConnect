...
bool CInputStreamPVRRecording::OpenPVRStream()
{
  ...
  if (!recording)
    CLog::Log(
        LOGERROR,
        "CInputStreamPVRRecording - %s - unable to obtain recording instance for recording %s",
        __FUNCTION__, m_item.GetPath().c_str());

  if (recording && m_client && (m_client->OpenRecordedStream(recording) == PVR_ERROR_NO_ERROR))
  {
    CLog::Log(LOGDEBUG, "CInputStreamPVRRecording - %s - opened recording stream %s", __FUNCTION__,
...
void CInputStreamPVRRecording::ClosePVRStream()
{
  ...
  {
    CLog::Log(LOGDEBUG, "CInputStreamPVRRecording - %s - closed recording stream %s", __FUNCTION__,
              m_item.GetPath().c_str());
  }
}
