...
bool CDVDInputStreamBluray::Open()
{
  ...
  CLog::Log(LOGDEBUG, "CDVDInputStreamBluray::Open - opening {}", NlcUrl::GetRedacted(root));

  if (openStream)
  {
    if (!bd_open_stream(m_bd, m_pstream.get(), read_blocks))
    {
      LogModule( eLogPlayerNlc, LOG_ERROR, "CDVDInputStreamBluray::Open - failed to open %s in stream mode",
                NlcUrl::GetRedacted(root));
      return false;
    }
  }
  else if (openDisc)
...
bool CDVDInputStreamBluray::OpenStream(CFileItem &item)
{
  ...
  if (!m_pstream->Open())
  {
    CLog::Log(LOGERROR, "Error opening image file {}", NlcUrl::GetRedacted(item.GetPath()));
    Close();
    return false;
  }
