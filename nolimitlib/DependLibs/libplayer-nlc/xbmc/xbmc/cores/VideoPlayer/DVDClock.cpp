...
void CDVDClock::SetSpeed( int iSpeed )
{
	...
	if( m_pauseClock )
	{
		int64_t diff = current - m_pauseClock;
		m_startClock += diff;
		//LogModule( eLogVideoIo, LOG_DEBUG, "CDVDClock::SetSpeed diff %lld m_pauseClock %lld m_startClock %lld", 
		//		   diff, m_pauseClock, m_systemUsed );
		m_pauseClock = 0;		
	}

	m_startClock = current - (int64_t)((double)(current - m_startClock) * newfreq / m_systemUsed);
	m_systemUsed = newfreq;
	if( m_systemUsed != 10000000 || iSpeed != 1000 )
	{
		LogModule( eLogVideoIo, LOG_DEBUG, "CDVDClock::SetSpeed speed %d m_systemUsed %lld", iSpeed, m_systemUsed );
	}
}
...
void CDVDClock::Discontinuity( double clock, double absolute )
{
	...
	LogModule( eLogVideoIo, LOG_DEBUG, "CDVDClock::Discontinuity clock %3.3f, absolute %3.3f", clock, absolute );
}
...
void CDVDClock::SetMaxSpeedAdjust( double speed )
{
	...
	if( speed != 0.0 )
	{
		LogModule( eLogVideoIo, LOG_DEBUG, "CDVDClock::SetMaxSpeedAdjust speed %3.3f", speed );
	}
}
...
int CDVDClock::UpdateFramerate( double fps, double* interval /*= NULL*/ )
{
	...
	LogModule( eLogVideoIo, LOG_DEBUG, "CDVDClock::UpdateFramerate fps %3.3f rate %3.3f", fps, rate );
	CSingleLock lock( m_speedsection );
	...
	{
		LogModule( eLogVideoIo, LOG_DEBUG, "CDVDClock::UpdateFramerate m_maxspeedadjust %3.3f", m_maxspeedadjust );
...
double CDVDClock::SystemToPlaying( int64_t system )
{
	...
	if( m_pauseClock )
	{
		//LogModule( eLogVideoIo, LOG_DEBUG, "CDVDClock::SystemToPlaying using pause clock %lld instead of system %lld",
		//		   m_pauseClock, system );
	...
	//LogModule( eLogVideoIo, LOG_DEBUG, "CDVDClock::SystemToPlaying current %lld m_iDisc %3.3f start %lld m_systemAdjust %lld m_systemUsed %lld",
	//		   current, m_iDisc, m_systemAdjust, m_systemUsed  );
