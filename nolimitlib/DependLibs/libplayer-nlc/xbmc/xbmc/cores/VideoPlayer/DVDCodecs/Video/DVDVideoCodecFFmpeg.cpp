...
//#endif
  ...
  if (!avpkt)
  {
    CLog::Log(LOGERROR, "CDVDVideoCodecFFmpeg::%s - av_packet_alloc failed: %s", __FUNCTION__,
              strerror(errno));
    return false;
  }
...
void CDVDVideoCodecFFmpeg::SetCodecControl( int flags )
{
    ...
    if( m_pCodecContext )
    {
        ...
        {
            lastFlags = flags;
            LogModule( eLogVideoIo, LOG_VERBOSE, " CDVDVideoCodecFFmpeg::SetCodecControl 0x%X drop %d", flags, bDrop );
        }
    }
