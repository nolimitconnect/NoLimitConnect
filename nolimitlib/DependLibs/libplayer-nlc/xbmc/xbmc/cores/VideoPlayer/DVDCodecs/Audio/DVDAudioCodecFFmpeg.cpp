...
int CDVDAudioCodecFFmpeg::GetData( uint8_t** dst )
{
    ...
    if( !ret )
    {
        ...
        */
        int channels = GetChannels(); // patch for old channels to work
...
{
    //return m_pCodecContext->ch_layout.nb_channels; // new channels
    return m_pCodecContext->channels; // old channels
}
...
void CDVDAudioCodecFFmpeg::BuildChannelMap()
{
    /* new channels
      ...
      if (bits == codecChannels)
        ...
        {
         LogModule( eLogPlayerNlc, LOG_ERROR,
                  "CDVDAudioCodecFFmpeg::GetChannelMap - FFmpeg reported %d channels, but the layout "
                  "contains %d - trying hints",
                  codecChannels, bits);
    ...
    */
    ...
    //m_channels = codecChannels; // new channels
    m_channels = m_pCodecContext->channels; // old channels
}
