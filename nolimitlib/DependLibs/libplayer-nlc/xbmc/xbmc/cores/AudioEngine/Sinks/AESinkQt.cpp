...
//============================================================================
std::unique_ptr<IAESink> CAESinkQt::Create( std::string &device, AEAudioFormat& desiredFormat )
{
    auto sink = std::make_unique<CAESinkQt>();
...
void CAESinkQt::GetDelay( AEDelayStatus& status )
{
    ...
    status.SetDelay( m_INlc.toGuiGetAudioDelaySeconds( eAppModulePlayerNlc ) );
}
...
double CAESinkQt::GetCacheTotal( )
{
    ...
    return  m_INlc.toGuiGetAudioCacheTotalSeconds( eAppModulePlayerNlc );
}
...
unsigned int CAESinkQt::AddPackets( uint8_t **data, unsigned int frames, unsigned int offset )
{
	...
	int retryCnt = 0;
    while( ( m_INlc.toGuiGetAudioCacheFreeSpace( eAppModulePlayerNlc ) < totalBytes ) && ( retryCnt < 50 ) )
    {
        int requiredSpace = totalBytes - m_INlc.toGuiGetAudioCacheFreeSpace( eAppModulePlayerNlc );
        if( requiredSpace > 0 )
        {
            VxSleep( 10 );
			retryCnt++;
...
//#ifdef DEBUG_KODI_AUDIO
	if( retryCnt >= 50 )
	{
		LogMsg( LOG_DEBUG, "CAESinkQt::AddPackets timeout snd buf needs %d free space but has %d", totalBytes, m_INlc.toGuiGetAudioCacheFreeSpace( eAppModulePlayerNlc )  );
	}
