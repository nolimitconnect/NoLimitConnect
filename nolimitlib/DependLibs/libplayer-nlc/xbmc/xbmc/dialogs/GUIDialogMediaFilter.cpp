...
void CGUIDialogMediaFilter::InitializeSettings()
{
  ...
  for (const Filter& f : filterList)
  {
    ...
    else
    {
      ...
      CLog::Log(LOGWARNING,
                "CGUIDialogMediaFilter: filter %d of media type %s with unknown control type '%s'",
    ...
    if (filter.setting == NULL)
    {
      ...
      CLog::Log(LOGWARNING,
                "CGUIDialogMediaFilter: failed to create filter %s of media type %s with control "
                "type '%s'",
                filter.field, filter.mediaType.c_str(), filter.controlType.c_str());
      continue;
    }
...
bool CGUIDialogMediaFilter::SetPath(const std::string &path)
{
  ...
  else
  {
    CLog::Log(
        LOGWARNING,
        "CGUIDialogMediaFilter::SetPath(%s): invalid path (neither videodb:// nor musicdb://)",
        path.c_str());
    return false;
  }
