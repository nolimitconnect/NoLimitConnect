...
void CLangInfo::OnSettingChanged(const std::shared_ptr<const CSetting>& setting)
{
  ...
  else if (settingId == CSettings::SETTING_LOCALE_USE24HOURCLOCK)
  {
    ...
    settings->SetString(CSettings::SETTING_LOCALE_TIMEFORMAT,
                        PrepareTimeFormat(GetLanguageTimeFormat(), m_use24HourClock));
  }
...
bool CLangInfo::Load(const std::string& strLanguage)
{
  ...
  {
    CLog::Log(LOGERROR, "unable to load %s: %s at line %d", strFileName.c_str(), xmlDoc.ErrorDesc(),
              xmlDoc.ErrorRow());
    return false;
  }
...
bool CLangInfo::SetLanguage(std::string language /* = "" */, bool reloadServices /* = true */)
{
                         ...
                         ADDON::OnlyEnabled::CHOICE_YES))
  {
        ...
        (addonMgr.IsAddonDisabled(language) && !addonMgr.EnableAddon(language)))
    {
      ...
      {
        CLog::Log(LOGFATAL, "CLangInfo::%s: could not find default language add-on '%s'", __func__,
                  language.c_str());
        return false;
      }
    }
...
const std::string& CLangInfo::GetLanguageDateFormat(bool bLongDate /* = false */) const
{
  if (bLongDate)
...
const std::string& CLangInfo::GetLanguageTimeFormat() const
{
  return m_timeFormat;
}
