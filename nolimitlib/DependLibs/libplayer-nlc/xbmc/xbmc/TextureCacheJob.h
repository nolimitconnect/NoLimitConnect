...
public:
   ...
   */
  bool CacheTexture(std::unique_ptr<CTextureBase>* texture = nullptr);
...
private:
  ...
  /*! \brief Load an image at a given target size and orientation.
   ...
   \return a pointer to a CTextureBase object, NULL if failed.
   */
  static std::unique_ptr<CTextureBase> LoadImage(const std::string& image,
                                             unsigned int width,
                                             unsigned int height,
