  ...
  : m_type(type), m_instanceId(instanceId), m_parentInstance(parentInstance), m_addonInfo(addonInfo)
{
  ...
  m_uniqueWorkID = !uniqueWorkID.empty() ? uniqueWorkID : StringUtils::Format("%p", (void *)this);
...
ADDON_STATUS IAddonInstanceHandler::CreateInstance()
{
  ...
  if (status != ADDON_STATUS_OK)
  {
    CLog::Log(LOGERROR,
              ...
              __func__, m_addon->ID().c_str(), kodi::addon::TranslateAddonStatus(status).c_str());
  }
  return status;
}
...
std::shared_ptr<CSetting> IAddonInstanceHandler::GetSetting(const std::string& setting)
{
  ...
  {
    CLog::Log(LOGERROR, "IAddonInstanceHandler::%s - couldn't get settings for add-on '%s'",
              __func__, Name().c_str());
    return nullptr;
  }
  ...
  if (value == nullptr)
  {
    CLog::Log(LOGERROR, "IAddonInstanceHandler::%s - can't find setting '%s' in '%s'", __func__,
              setting.c_str(), Name().c_str());
    return nullptr;
  }
                                                      ...
                                                      bool* value)
{
  ...
  {
    CLog::Log(LOGERROR, "IAddonInstanceHandler::%s - setting '%s' is not a boolean in '%s'",
              __func__, id, instance->Name().c_str());
    return false;
  }
                                                     ...
                                                     int* value)
{
  ...
  {
    CLog::Log(LOGERROR, "IAddonInstanceHandler::%s - setting '%s' is not a integer in '%s'",
              __func__, id, instance->Name().c_str());
    return false;
  }
                                                       ...
                                                       float* value)
{
  ...
  {
    CLog::Log(LOGERROR, "IAddonInstanceHandler::%s - setting '%s' is not a number in '%s'",
              __func__, id, instance->Name().c_str());
    return false;
  }
                                                        ...
                                                        char** value)
{
  ...
  {
    CLog::Log(LOGERROR, "IAddonInstanceHandler::%s - setting '%s' is not a string in '%s'",
              __func__, id, instance->Name().c_str());
    return false;
  }
