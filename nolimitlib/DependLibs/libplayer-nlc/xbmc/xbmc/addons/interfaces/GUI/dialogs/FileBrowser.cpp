                                                            ...
                                                            bool write_only)
{
  ...
  if (!addon)
  {
    CLog::Log(LOGERROR, "Interface_GUIDialogFileBrowser::%s - invalid data", __func__);
    return false;
  }
  ...
  if (!shares || !heading || !path_in || !path_out)
  {
    CLog::Log(LOGERROR,
              ...
              static_cast<const void*>(path_in), static_cast<void*>(path_out), addon->ID().c_str());
    return false;
  }
                                                             ...
                                                             unsigned int* entries)
{
  ...
  if (!shares || !heading || !file_list || !entries)
  {
    CLog::Log(LOGERROR,
              ...
              "heading='%p', file_list='%p', entries='%p') on addon '%s'",
              __func__, static_cast<const void*>(shares), static_cast<const void*>(heading),
              static_cast<void*>(file_list), static_cast<void*>(entries), addon->ID().c_str());
    return false;
  }
                                                     ...
                                                     unsigned int entries)
{
  ...
  else
  {
    ...
    CLog::Log(LOGERROR,
              "Interface_GUIDialogFileBrowser::%s - invalid handler data (file_list='%p') on "
              "addon '%s'",
              __func__, static_cast<void*>(file_list), addon->ID().c_str());
  }
}
