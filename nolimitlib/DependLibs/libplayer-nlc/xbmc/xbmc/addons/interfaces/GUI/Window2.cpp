                                                   ...
                                                   bool is_media)
{
  ...
  {
    CLog::Log(LOGERROR,
              "Interface_GUIWindow::%s - invalid handler data (xml_filename='%p', "
              "default_skin='%p') on addon '%s'",
              __func__, static_cast<const void*>(xml_filename),
                static_cast<const void*>(default_skin), addon ? addon->ID().c_str() : "unknown");
    return nullptr;
  }
  ...
  {
    CLog::Log(LOGWARNING,
              "Interface_GUIWindow::%s: %%s/%s - addon tries to create dialog as media window who "
              "not allowed, contact Developer '%s' of this addon",
              __func__, CAddonInfo::TranslateType(addon->Type()).c_str(), addon->Name().c_str(), addon->Author().c_str());
  }
  ...
  if (!CFileUtils::Exists(strSkinPath))
  {
    ...
    if (!CFileUtils::Exists(strSkinPath))
    {
      ...
      {
        CLog::Log(LOGERROR,
                  "Interface_GUIWindow::%s: %s/%s - XML File '%s' for Window is missing, contact "
                  "Developer '%s' of this addon",
                    __func__, CAddonInfo::TranslateType(addon->Type()).c_str(), addon->Name().c_str(), strSkinPath.c_str(),
                    addon->Author().c_str());
        return nullptr;
      }
    }
  ...
  {
    CLog::Log(LOGERROR,
              "Interface_GUIWindow::%s - Requested window id '%d' does not exist for addon '%s'",
              __func__, id, addon->ID().c_str());
    delete window;
    return nullptr;
  }
...
void Interface_GUIWindow::destroy(KODI_HANDLE kodiBase, KODI_GUI_WINDOW_HANDLE handle)
{
  ...
  {
    CLog::Log(LOGERROR,
              "Interface_GUIWindow::%s - invalid handler data (kodiBase='%p', "
              "handle='%p') on addon '%s'",
    ...
    bool (*CBOnContextButton)(KODI_GUI_CLIENT_HANDLE, int, unsigned int))
{
  ...
  {
    CLog::Log(LOGERROR,
              "Interface_GUIWindow::%s - invalid handler data (handle='%p', clienthandle='%p') "
              "on addon '%s'",
              __func__, handle, clienthandle, addon ? addon->ID().c_str() : "unknown");
    return;
  }
                                       ...
                                       int control_id)
{
  ...
  {
    CLog::Log(LOGERROR, "Interface_GUIWindow - %s: %s - Control does not exist in window", __func__,
              addon->Name().c_str());
    return false;
  }
                                              ...
                                              bool visible)
{
  ...
  {
    CLog::Log(LOGERROR,
              "Interface_GUIWindow::%s - invalid handler data (kodiBase='%p', handle='%p') on "
              "addon '%s'",
              __func__, kodiBase, handle, addon ? addon->ID().c_str() : "unknown");
    return;
  }
                                       ...
                                       const char* value)
{
  ...
  if (!addon || !pAddonWindow || !key || !value)
  {
    CLog::Log(LOGERROR,
              ...
              static_cast<const void*>(value), addon ? addon->ID().c_str() : "unknown");
    return;
  }
                                        ...
                                        int list_position)
{
  ...
  {
    CLog::Log(LOGERROR, "Interface_GUIWindow::%s - empty list item called on addon '%s'", __func__,
              addon->ID().c_str());
    return;
  }
                                                            ...
                                                            int list_position)
{
  ...
  {
    CLog::Log(LOGERROR, "ADDON::Interface_GUIWindow - %s: %s - Index out of range", __func__,
              addon->Name().c_str());
                                              ...
                                              const std::string& typeName)
{
  ...
  if (!m_windowLoaded)
  {
    ...
    {
      CLog::Log(LOGERROR,
                "CGUIAddonGUI_Window::%s: %s - Window initialize failed by control id '%d' request "
                "for '%s'",
                __func__, m_addon->Name().c_str(), controlId, typeName.c_str());
      return nullptr;
    }
  }
  ...
  {
    CLog::Log(LOGERROR,
              "CGUIAddonGUI_Window::%s: %s - Requested GUI control Id '%d' for '%s' not present!",
              __func__, m_addon->Name().c_str(), controlId, typeName.c_str());
    return nullptr;
  }
  ...
  {
    CLog::Log(LOGERROR,
              "CGUIAddonGUI_Window::%s: %s - Requested GUI control Id '%d' not the type '%s'!",
              __func__, m_addon->Name().c_str(), controlId, typeName.c_str());
    return nullptr;
  }
