                                                         ...
                                                         int retries)
{
  ...
  if (!addon)
  {
    CLog::Log(LOGERROR, "Interface_GUIDialogNumeric::%s - invalid data", __func__);
    return -1;
  }
                                                   ...
                                                   const char* heading)
{
  ...
  {
    CLog::Log(LOGERROR,
              "Interface_GUIDialogNumeric::%s - invalid handler data (time='%s', heading='%s') on "
              "addon '%s'",
              __func__, static_cast<void*>(time), static_cast<const void*>(heading), addon->ID().c_str());
    return false;
  }
                                                   ...
                                                   const char* heading)
{
  ...
  if (!date || !heading)
  {
    CLog::Log(LOGERROR,
              ...
              "addon '%s'",
              __func__, static_cast<void*>(date), static_cast<const void*>(heading), addon->ID().c_str());
    return false;
  }
