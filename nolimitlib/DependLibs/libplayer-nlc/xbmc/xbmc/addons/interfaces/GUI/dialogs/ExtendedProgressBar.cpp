                                                                ...
                                                                const char* title)
{
  ...
  if (!title || !dialog)
  {
    CLog::Log(LOGERROR,
              ...
              "dialog='%p') on addon '%s'",
              __func__, static_cast<const void*>(title), static_cast<void*>(dialog), addon->ID().c_str());
    return nullptr;
  }
                                                        ...
                                                        KODI_GUI_HANDLE handle)
{
  ...
  if (!addon)
  {
    CLog::Log(LOGERROR, "Interface_GUIDialogExtendedProgress::%s - invalid kodi base data",
              __func__);
    return;
  }
  ...
  {
    CLog::Log(LOGERROR,
              "Interface_GUIDialogExtendedProgress::%s - invalid handler data (kodiBase='%p', "
              "handle='%p') on addon '%s'",
...
char* Interface_GUIDialogExtendedProgress::get_title(KODI_HANDLE kodiBase, KODI_GUI_HANDLE handle)
{
  ...
  if (!handle)
  {
    CLog::Log(LOGERROR,
              ...
              "addon '%s'",
              __func__, handle, addon->ID().c_str());
    return nullptr;
  }
                                                    ...
                                                    const char* title)
{
  ...
  if (!handle || !title)
  {
    CLog::Log(LOGERROR,
              ...
              "title='%p') on addon '%s'",
              __func__, handle, static_cast<const void*>(title), addon->ID().c_str());
    return;
  }
...
char* Interface_GUIDialogExtendedProgress::get_text(KODI_HANDLE kodiBase, KODI_GUI_HANDLE handle)
{
  ...
  {
    CLog::Log(LOGERROR,
              "Interface_GUIDialogExtendedProgress::%s - invalid add-on data (handle='%p') on "
              "addon '%s'",
              __func__, handle, addon->ID().c_str());
    return nullptr;
  }
                                                   ...
                                                   const char* text)
{
  ...
  if (!handle || !text)
  {
    CLog::Log(LOGERROR,
              "Interface_GUIDialogExtendedProgress::%s - invalid handler data (handle='%p', "
              "text='%p') on addon '%s'",
              __func__, handle, static_cast<const void*>(text), addon->ID().c_str());
    return;
  }
