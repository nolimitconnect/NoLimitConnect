...
char* Interface_Base::get_addon_info(const KODI_ADDON_BACKEND_HDL hdl, const char* id)
{
  ...
  {
    CLog::Log(LOGERROR, "Interface_Base::%s -  add-on '%s' requests invalid id '%s'", __func__,
              addon->Name().c_str(), id);
    return nullptr;
  }
...
bool Interface_Base::open_settings_dialog(const KODI_ADDON_BACKEND_HDL hdl)
{
  ...
  {
    CLog::Log(LOGERROR, "Interface_Base::%s - Could not get addon information for '%s'", __func__,
              addon->ID().c_str());
    return false;
  }
...
bool Interface_Base::is_setting_using_default(const KODI_ADDON_BACKEND_HDL hdl, const char* id)
{
  ...
  if (addon == nullptr || id == nullptr)
  {
    CLog::Log(LOGERROR, "Interface_Base::%s - invalid data (addon='%p', id='%p')", __func__, hdl,
  ...
  {
    CLog::Log(LOGERROR, "Interface_Base::%s - couldn't get settings for add-on '%s'", __func__,
              addon->Name().c_str());
    return false;
  }
