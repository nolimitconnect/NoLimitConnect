...
std::vector<std::string> CAddonInstaller::RemoveOrphanedDepsRecursively() const
{
  ...
  while (toRemove.size() > 0)
  {
    for (const auto& dep : toRemove)
    {
      ...
      {
        CLog::Log(LOGERROR, "CAddonMgr::{}: failed to remove orphaned add-on/dependency: %s",
                  __func__, dep->Name().c_str());
      }
    }
...
bool CAddonInstaller::InstallFromZip(const std::string &path)
{
      ...
      items.Size() != 1 || !items[0]->m_bIsFolder)
  {
    ...
    CLog::Log(
        LOGERROR,
        "CAddonInstaller: installing addon failed '%s' - itemsize: %d, first item is folder: %d",
        NlcUrl::GetRedacted(path).c_str(), items.Size(), items[0]->m_bIsFolder);
    return false;
  }
