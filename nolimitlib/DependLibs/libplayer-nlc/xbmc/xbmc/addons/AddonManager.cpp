...
bool CAddonMgr::Init()
{
  ...
  if (!LoadManifest(m_systemAddons, m_optionalSystemAddons))
  {
...
#if !ENABLE_GAMES
    removeAddonList.push_back("game");
#endif // ENABLE_GAMES
    ...
    {
      // if games are not enable then remove any game related addons
      for( auto iter = m_systemAddons.begin(); iter != m_systemAddons.end(); )
      {
          if( iter->find( addonName ) != std::string::npos )
          {
              iter = m_systemAddons.erase( iter );
          }
          else
          {
              iter++;
    ...
    AddonCheckType addonCheckType) const
{
  ...
  CLog::Log(LOGDEBUG, "CAddonMgr::GetAvailableUpdatesOrOutdatedAddons took %d ms",
            duration.count());
...
std::map<std::string, AddonWithUpdate> CAddonMgr::GetAddonsWithAvailableUpdate() const
{
  ...
  auto end = std::chrono::steady_clock::now();
  auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
  CLog::Log(LOGDEBUG, "CAddonMgr::%s took %d ms", __func__, duration.count());
...
bool CAddonMgr::UnloadAddon(const std::string& addonId)
{
  ...
  {
    CLog::Log(LOGERROR, "CAddonMgr::%s: could not unload binary add-on %s, as is in use", __func__,
              addonId.c_str());
    return false;
  }

  m_installedAddons.erase(addonId);
  CLog::Log(LOGDEBUG, "CAddonMgr::%s: %s unloaded", __func__, addonId.c_str());
...
bool CAddonMgr::UpdateDisabledReason(const std::string& id, AddonDisabledReason newDisabledReason)
{
  ...
  // success
  CLog::Log(LOGDEBUG, "CAddonMgr: DisabledReason for %s updated to %d", id.c_str(),
      ...
      if (CFileUtils::Exists(path + "addon.xml"))
      {
        ...
        if (addonInfo)
        {
          ...
          if (it != addonmap.end())
          {
            ...
            CLog::Log(LOGWARNING, "CAddonMgr::%s: Addon '%s' present with version %s at '%s' - other version %s at '%s' will be ignored",
                      __FUNCTION__, addonInfo->ID().c_str(), it->second->Version().asString().c_str(), it->second->Path().c_str(), addonInfo->Version().asString().c_str(), addonInfo->Path().c_str());
          }
                                  ...
                                  std::vector<AddonInfoPtr>& addons)
{
  ...
  if (!doc.Parse(xml))
  {
    CLog::Log(LOGERROR, "CAddonMgr::%s: Failed to parse addons.xml", __func__);
    return false;
  }

  if (doc.RootElement() == nullptr || doc.RootElement()->ValueStr() != "addons")
  {
    CLog::Log(LOGERROR, "CAddonMgr::%s: Failed to parse addons.xml. Malformed", __func__);
    return false;
  }
