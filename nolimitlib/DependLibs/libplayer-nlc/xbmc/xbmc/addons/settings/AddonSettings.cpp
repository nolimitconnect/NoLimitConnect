                                       ...
                                       const Logger& logger)
{
  ...
  if (setting == nullptr)
  {
    logger->warn("failed to create setting \"%s\" on-the-fly", settingId.c_str());
    return nullptr;
  }
  ...
  {
    logger->warn("failed to add setting \"%s\" on-the-fly", settingId.c_str());
    return nullptr;
  }
...
CAddonSettings::CAddonSettings(const std::shared_ptr<const IAddon>& addon,
  ...
  : CSettingsBase(),
    ...
    m_logger(CServiceBroker::GetLogging().GetLogger(
        StringUtils::Format("CAddonSettings[%d@%s]", m_instanceId, m_addonId.c_str())))
{
}
...
bool CAddonSettings::Load(const CXBMCTinyXML& doc)
{
  ...
  for (const auto& setting : settingValues)
  {
    ...
    if (!newSetting)
    {
      m_logger->error("had null newSetting for value \"%s\" for setting %s", setting.second.c_str(),
                      setting.first.c_str());
    }
    else if (!newSetting->FromString(setting.second))
    {
      m_logger->warn("failed to load value \"%s\" for setting %s", setting.second.c_str(), setting.first.c_str());
    }
  }
    ...
    std::set<std::string>& settingIds)
{
  ...
  while (settingElement != nullptr)
  {
    ...
    {
      m_logger->warn("failed to parse old setting definition for \"%s\" of type \"%s\"", settingId.c_str(),
                     settingType.c_str());
    }
    ...
    const std::vector<std::string>& settingLValues)
{
  ...
  else
  {
    ...
    if (!source.empty())
      ...
      m_logger->warn("failed to parse old setting definition for \"%s\" of type \"select\"",
                     settingId.c_str());
  }
                                                         ...
                                                         const int settingLabel)
{
  ...
  if (addonTypes.empty())
  {
    m_logger->error("missing addon type for addon setting \"%s\"", settingId.c_str());
    return nullptr;
  }
  ...
  {
    m_logger->error("multiple addon types are not supported (addon setting \"%s\")", settingId.c_str());
    return nullptr;
  }
  ...
  {
    m_logger->warn("multiple default addon ids on non-multiselect addon setting \"%s\"", settingId.c_str());
    ...
    const std::vector<std::string>& settingLValues)
{
  ...
  else if (settingValues == "$HOURS")
  {
    for (uint32_t hour = 0; hour < 24; hour++)
      values.push_back(
          CDateTime(2000, 1, 1, hour, 0, 0).GetAsLocalizedTime(g_langInfo.GetLanguageTimeFormat(), false));
  }
  else
    values = StringUtils::Split(settingValues, OldSettingValuesSeparator);
                                                          ...
                                                          const std::string& defaultValue)
{
  ...
  m_logger->warn("ignoring old setting definition for \"%s\" of type \"slider\" because of unknown "
                 "option \"%s\"",
                                       ...
                                       CSettingDependency& dependeny) const
{
  ...
  {
    m_logger->warn("failed to parse old setting conditions \"%s\" for \"%s\"", condition.c_str(),
                   setting->GetId().c_str());
    return false;
  }
  ...
  for (const auto& cond : conditions)
  {
    ...
    {
      m_logger->warn("cannot reference setting (relative index: %d; absolute index: %d) in another "
