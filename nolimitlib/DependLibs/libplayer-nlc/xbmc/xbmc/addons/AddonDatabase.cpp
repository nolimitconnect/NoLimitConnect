...
bool CAddonDatabase::SetLastUsed(const std::string& addonId, const CDateTime& dateTime)
{
  try
  {
    ...
    CLog::Log(LOGDEBUG, "CAddonDatabase::SetLastUsed[%s] took %d ms", addonId.c_str(), duration.count());
    return true;
  }
  catch (...)
  {
...
bool CAddonDatabase::GetRepositoryContent(const std::string& id, VECADDONS& addons) const
{
  try
  {
    ...
    auto end = std::chrono::steady_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
    CLog::Log(LOGDEBUG, "CAddonDatabase: SELECT repo.id FROM repo .. took %d ms", duration.count());
