                                         ...
                                         const std::shared_ptr<IAddon>& repoAddon)
{
  ...
  {
    CLog::LogFC(LOGDEBUG, LOGADDONS, "%s - %d addon(s) loaded", repo.first.c_str(), repo.second.size());
                                            ...
                                            AddonCheckType addonCheckType) const
{
  ...
  CLog::LogFC(LOGDEBUG, LOGADDONS, "Building %s list from installed add-ons",
...
{
  CLog::LogFC(LOGDEBUG, LOGADDONS, "update check: addonID = %s / Origin = %s / Version = %s",
  ...
  if (update != nullptr)
  {
    CLog::LogFC(LOGDEBUG, LOGADDONS, "-- found -->: addonID = %s / Origin = %s / Version = %s",
                                 ...
                                 std::shared_ptr<CRepository>& repoForDep) const
{
  ...
  CLog::LogFC(LOGDEBUG, LOGADDONS, "found dependency [%s] for install/update from repo [%s]",
