...
AddonInfoPtr CAddonInfoBuilder::Generate(const std::string& addonPath, bool platformCheck /*= true*/)
{
  ...
  {
    CLog::Log(LOGERROR, "CAddonInfoBuilder::%s: Unable to load '%s', Line %d\n%s",
                                               __FUNCTION__,
                                 ...
                                 const RepositoryDirInfo& repo)
{
  ...
  {
    CLog::Log(LOGERROR, "CAddonInfoBuilder::%s: identifier %d is invalid", __FUNCTION__, addon->m_id.c_str());
    return false;
  }
  ...
  if (child)
  {
    ...
    if (library != nullptr)
    {
      ...
      try
      {
        ...
        {
          info->SetBinary(true);
          CLog::Log(LOGDEBUG, "CAddonInfoBuilder::%s: Binary addon found: %s", __func__,
                    info->ID().c_str());
        }
      }
      catch (const std::regex_error& e)
      {
        CLog::Log(LOGERROR, "CAddonInfoBuilder::%s: Regex error caught: %s", __func__,
                  e.what());
      }
    }
