...
CScraperUrl CScraper::NfoUrl(const std::string &sNfoContent)
{
  ...
  if (m_isPython)
  {
    ...
    str << "plugin://" << ID() << "?action=NfoUrl&nfo=" << NlcUrl::Encode(sNfoContent)
        << "&pathSettings=" << NlcUrl::Encode(GetPathSettingsAsJSON());
                          ...
                          T &result)
{
  ...
  str << "&pathSettings=" << NlcUrl::Encode(pathSettings);
  ...
  for (std::vector<std::string>::const_iterator i = vcsOut.begin(); i != vcsOut.end(); ++i)
  {
         ...
         pxeMovie = pxeMovie->NextSiblingElement())
    {
      ...
      if (pxnTitle && pxnTitle->FirstChild() && pxeLink && pxeLink->FirstChild())
      {
        ...
        scurlMovie.SetRelevance(fstrcmp(sMatchTitle.c_str(), sCompareTitle.c_str(), 0) + yearScore);
...
EPISODELIST CScraper::GetEpisodeList(XFILE::CCurlFile &fcurl, const CScraperUrl &scurl)
{
  ...
  if (m_isPython)
  {
    std::stringstream str;
    str << "plugin://" << ID()
        << "?action=getepisodelist&url=" << NlcUrl::Encode(scurl.GetFirstThumbUrl())
        << "&pathSettings=" << NlcUrl::Encode(GetPathSettingsAsJSON());
