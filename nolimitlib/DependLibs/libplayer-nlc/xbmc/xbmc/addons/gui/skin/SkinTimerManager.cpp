...
void CSkinTimerManager::LoadTimers(const std::string& path)
{
  ...
  {
      CLog::LogF(LOGWARNING, "Could not load timers file %s: %s (row: %d, col: %d)", path.c_str(),
  ...
  {
    CLog::LogF(LOGERROR, "Error loading timers file %s: Root element <timers> required.", path.c_str());
    return;
  }
...
bool CSkinTimerManager::TimerIsRunning(const std::string& timer) const
{
  ...
  {
    CLog::LogF(LOGERROR, "Couldn't find Skin Timer with name: %s", timer.c_str());
    return false;
  }
