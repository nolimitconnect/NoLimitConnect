...
void CPowerManager::SetDefaults()
{
  ...
  if (!setting)
    {
    CLog::Log(LOGERROR, "Failed to load setting for: %d",
...
void CPowerManager::RestorePlayerState()
{
  ...
  CLog::Log(LOGDEBUG, "CPowerManager::RestorePlayerState - resume last played item (startOffset: %i ms)", m_lastPlayedFileItem->GetStartOffset());
