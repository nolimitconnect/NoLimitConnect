...
bool CFile::Open( const NlcUrl& file, const unsigned int flags )
{
	...
	m_flags = flags;
	if( m_flags & READ_VIRT_STREAM_FILE )
	{
		m_pFile.reset( new CVirtualStreamFile() );
		if( !m_pFile->Open( file ) )
		{
			return false;
		}
...
ssize_t CFile::LoadFile( const NlcUrl& file, std::vector<uint8_t>& outputBuffer )
{
