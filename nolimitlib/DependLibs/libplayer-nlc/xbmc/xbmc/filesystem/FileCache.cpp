...
bool CFileCache::Open(const NlcUrl& url)
{
  ...
  CLog::Log(LOGDEBUG,
            "CFileCache::%s - <%s> source chunk size is %d, setting cache chunk size to %d",
  ...
  if (!m_pCache)
  {
    ...
    else
    {
      ...
      if (m_flags & READ_MULTI_STREAM)
        CLog::Log(LOGDEBUG, "CFileCache::%s - <%s> using double memory cache each sized %d bytes",
                  __FUNCTION__, m_sourcePath.c_str(), cacheSize);
      else
        CLog::Log(LOGDEBUG, "CFileCache::%s - <%s> using single memory cache sized %d bytes",
                  __FUNCTION__, m_sourcePath.c_str(), cacheSize);
  ...
  while (!m_bStop)
  {
    ...
    if (m_seekEvent.Wait(0ms))
    {
      ...
      if (!sourceSeekFailed)
      {
        ...
        {
          CLog::Log(LOGDEBUG,
                    "CFileCache::%s - <%s> cache completely reset ",
    ...
    if (iRead <= 0)
    {
      ...
      {
        CLog::Log(LOGWARNING, "CFileCache::%s - <%s> source read returned %d! Will retry",
      ...
      else
      {
        if (iRead < 0)
          CLog::Log(LOGERROR,
                    "%s - <%s> source read failed with %d!", __FUNCTION__, m_sourcePath.c_str(), iRead);
