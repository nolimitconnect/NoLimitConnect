...
namespace KODI
{
	namespace MESSAGING
	{
		...
		void CApplicationMessenger::ProcessMessage( ThreadMessage* pMsg )
		{
			...
			{
				LogModule( eLogVideoIo, LOG_VERBOSE, "CApplicationMessenger::ProcessMessage callback %d", pMsg->dwMessage );
			...
			{
				if( TMSG_NETWORKMESSAGE != pMsg->dwMessage &&
					TMSG_RENDERER_UNINIT != pMsg->dwMessage )
				{
					CSingleExit exit( m_critSection );

					LogModule( eLogVideoIo, LOG_VERBOSE, "CApplicationMessenger::ProcessMessage OnApplicationMessage %d", pMsg->dwMessage );
					it->second->OnApplicationMessage( pMsg );
				}
			}
			else
				CLog::LogF( LOGERROR, "receiver %d is not defined", mask );
		}
