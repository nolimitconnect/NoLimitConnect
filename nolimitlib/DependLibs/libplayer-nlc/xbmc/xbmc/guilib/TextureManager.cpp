...
void CTextureArray::Add(std::shared_ptr<CTextureBase> texture, int delay)
{
  if (!texture)
    return;
...
void CTextureArray::Set(std::shared_ptr<CTextureBase> texture, int width, int height)
{
...
void CTextureMap::Dump() const
{
  ...
  CLog::Log(LOGDEBUG, "%s: texture:%s has %lld frames %u refcount", __FUNCTION__, m_textureName.c_str(),
...
void CTextureMap::Add(std::unique_ptr<CTextureBase> texture, int delay)
{
  if (texture)
    m_memUsage += sizeof(CTextureBase) + (texture->GetTextureWidth() * texture->GetTextureHeight() * 4);
...
const CTextureArray& CGUITextureManager::Load(const std::string& strTextureName, bool checkBundleOnly /*= false */)
{
           ...
           StringUtils::EndsWithNoCase(strPath, ".apng"))
  {
    ...
    while (frame)
    {
      std::unique_ptr<CTextureBase> glTexture = CTextureBase::CreateTexture();
      if (glTexture)
      {
      ...
      {
        CLog::Log(LOGDEBUG, "Memory limit (%lld bytes) exceeded, %u frames extracted from file : %s",
                  (maxMemoryUsage / 11) * 12, pMap->GetTexture().size(),
                  NlcUrl::GetRedacted(strPath).c_str());
        break;
      }
    }
  ...
  std::unique_ptr<CTextureBase> pTexture;
  ...
  {
    pTexture = CTextureBase::LoadFromFile(strPath);
    if (!pTexture)
      return emptyTexture;
