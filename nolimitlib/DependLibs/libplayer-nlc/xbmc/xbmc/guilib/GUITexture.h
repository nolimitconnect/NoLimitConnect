...
using CreateGUITextureFunc = std::function<CGUITextureBase*(
    float posX, float posY, float width, float height, const CTextureInfo& texture)>;
using DrawQuadFunc = std::function<void(
    const CRect& coords, UTILS::COLOR::Color color, CTextureBase* texture, const CRect* texCoords)>;
