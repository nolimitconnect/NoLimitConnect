...
bool CGUIWindow::Load(const std::string& strFileName, bool bContainsPath)
{
  ...
  if (ret)
  {
    ...
    CLog::Log(LOGDEBUG, "Skin file %s loaded in %3.2f ms", strPath.c_str(), duration.count());
#endif
  }
...
bool CGUIWindow::OnMessage(CGUIMessage& message)
{
  ...
  case GUI_MSG_WINDOW_DEINIT:
    {
      CLog::Log(LOGDEBUG, "------ Window Deinit (%d) ------", GetProperty("xmlfile").asString().c_str());
  ...
  case GUI_MSG_UNFOCUS_ALL:
    {
      ...
      if(control)
      {
        ...
        CLog::Log(LOGDEBUG, "Unfocus WindowID: %d, ControlID: %d", GetID(), control->GetID());
      }
      return true;
    }
...
void CGUIWindow::AllocResources(bool forceLoad /*= false */)
{
  ...
  if (forceLoad)
    CLog::Log(LOGDEBUG, "Alloc resources: %3.2f ms (%3.2f ms skin load)", duration.count(),
              skinLoadDuration.count());
  else
  {
    CLog::Log(LOGDEBUG, "Window %s was already loaded", GetProperty("xmlfile").asString().c_str());
    CLog::Log(LOGDEBUG, "Alloc resources: %3.2 ms", duration.count());
  }
#endif
  m_bAllocated = true;
}
