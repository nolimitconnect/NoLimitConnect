...
bool LoadXMLData(const std::string& filepath, CXBMCTinyXML& xmlDoc)
{
  ...
  {
    CLog::LogF(LOGDEBUG, "Couldn't load '%s' the file don't exists", filepath.c_str());
    return false;
  }
  if (!xmlDoc.LoadFile(filepath))
  {
    CLog::LogF(LOGERROR, "Couldn't load '%s'", filepath.c_str());
    return false;
  }
  TiXmlElement* pRootElement = xmlDoc.RootElement();
  if (!pRootElement || pRootElement->ValueStr() != "fonts")
  {
    CLog::LogF(LOGERROR, "Couldn't load '%s' XML content doesn't start with <fonts>", filepath.c_str());
    return false;
  }
  return true;
}
                                  ...
                                  bool preserveAspect)
{
  ...
  CGUIFontTTFBase* pFontFile = GetFontFile(fontIdent);
  if (!pFontFile)
  {
    pFontFile = CGUIFontTTFBase::CreateGUIFontTTF(fontIdent);
...
CGUIFontTTFBase* GUIFontManager::GetFontFile(const std::string& fontIdent)
{
    ...
    while (fontsetElement)
    {
      ...
      if (idAttr)
      {
        ...
        if (StringUtils::EqualsNoCase(fontSet, idAttr))
        {
          ...
          CLog::LogF(LOGINFO, "Loading <fontset> with name '%s' from '%s'", fontSet.c_str(),
  ...
  if (isCacheChanged)
  {
    ...
    if (fontsNode)
    {
      ...
      if (!xmlDoc.SaveFile(userFontCacheFilepath))
        CLog::LogF(LOGERROR, "Failed to save fonts cache file \"%s\"", userFontCacheFilepath.c_str());
    }
    else
    {
