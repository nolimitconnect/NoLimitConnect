...
std::string CStereoscopicsManager::DetectStereoModeByString(const std::string &needle) const
{
  ...
  {
    CLog::Log(
        LOGERROR, "%s: Invalid RegExp for matching 3d content:'%s'", __FUNCTION__,
        CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_stereoscopicregex_3d.c_str());
    return stereoMode;
  }
  ...
  {
    CLog::Log(
        LOGERROR, "%s: Invalid RegExp for matching 3d SBS content:'%s'", __FUNCTION__,
        CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_stereoscopicregex_sbs.c_str());
    return stereoMode;
  }
  ...
  {
    CLog::Log(
        LOGERROR, "%s: Invalid RegExp for matching 3d TAB content:'%s'", __FUNCTION__,
        CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_stereoscopicregex_tab.c_str());
    return stereoMode;
  }
...
RENDER_STEREO_MODE CStereoscopicsManager::GetStereoModeOfPlayingVideo(void) const
{
  ...
  CLog::Log(LOGDEBUG, "StereoscopicsManager: autodetected stereo mode for movie mode %s is: %s",
...
void CStereoscopicsManager::OnSettingChanged(const std::shared_ptr<const CSetting>& setting)
{
  ...
  {
    RENDER_STEREO_MODE mode = GetStereoMode();
    CLog::Log(LOGDEBUG, "StereoscopicsManager: stereo mode setting changed to %s",
...
void CStereoscopicsManager::ApplyStereoMode(const RENDER_STEREO_MODE &mode, bool notify)
{
  ...
  CLog::Log(LOGDEBUG,
            "StereoscopicsManager::ApplyStereoMode: trying to apply stereo mode. Current: %s | "
            "Target: %s",
  ...
  if (currentMode != mode)
  {
    ...
    CLog::Log(LOGDEBUG, "StereoscopicsManager: stereo mode changed to %s",
              ConvertGuiStereoModeToString(mode));
    if (notify)
