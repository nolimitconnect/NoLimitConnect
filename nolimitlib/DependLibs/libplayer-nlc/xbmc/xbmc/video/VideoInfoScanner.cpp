...
namespace VIDEO
{
      ...
      while (!bCancelled && !m_pathsToScan.empty())
      {
        ...
        else if (!CDirectory::Exists(directory))
        {
          ...
          CLog::Log(LOGWARNING, "%s directory '%s' does not exist - skipping scan%s.", __FUNCTION__,
                    NlcUrl::GetRedacted(directory).c_str(), m_bClean ? " and clean" : "");
  ...
  bool CVideoInfoScanner::DoScan(const std::string& strDirectory)
  {
    ...
    if (URIUtils::IsPlugin(strDirectory) && !CPluginDirectory::IsMediaLibraryScanningAllowed(TranslateContent(content), strDirectory))
    {
      CLog::Log(
          ...
          NlcUrl::GetRedacted(strDirectory), TranslateContent(content));
      return true;
    }
    ...
    if (content == CONTENT_MOVIES ||content == CONTENT_MUSICVIDEOS)
    {
      ...
      { // hash matches - skipping
        CLog::Log(LOGDEBUG, "VideoInfoScanner: Skipping dir '%s' due to no change%s",
                  NlcUrl::GetRedacted(strDirectory).c_str(), !fastHash.empty() ? " (fasthash)" : "");
        bSkip = true;
      }
      else if (hash.empty())
      ...
      { // hash changed - rescan
        CLog::Log(LOGDEBUG, "VideoInfoScanner: Rescanning dir '%s' due to change (%s != %s)",
                  NlcUrl::GetRedacted(strDirectory).c_str(), dbHash.c_str(), hash.c_str());
      }
    }
  ...
  bool CVideoInfoScanner::RetrieveVideoInfo(CFileItemList& items, bool bDirNames, CONTENT_TYPE content, bool useLocal, CScraperUrl* pURL, bool fetchEpisodes, CGUIDialogProgress* pDlgProgress)
  {
    ...
    for (int i = 0; i < items.Size(); ++i)
    {
      ...
      {
        CLog::Log(LOGERROR, "VideoInfoScanner: Unknown content type %d (%s)", info2->Content(),
                  NlcUrl::GetRedacted(pItem->GetPath()).c_str());
        FoundSomeInfo = false;
        break;
      }
      if (ret == INFO_CANCELLED || ret == INFO_ERROR)
      {
        CLog::Log(LOGWARNING,
                  ...
                  "information for %s.",
                  ret, NlcUrl::GetRedacted(pItem->GetPath()).c_str());
        FoundSomeInfo = false;
        break;
      }
      ...
      else if (ret == INFO_NOT_FOUND)
      {
        ...
        if (eventLog)
          eventLog->Add(EventPtr(new CMediaLibraryEvent(
              ...
              pItem->GetArt("thumb"), NlcUrl::GetRedacted(pItem->GetPath()), EventLevel::Warning)));
      }
  ...
  bool CVideoInfoScanner::EnumerateSeriesFolder(CFileItem* item, EPISODELIST& episodeList)
  {
    ...
    if (item->m_bIsFolder)
    {
      ...
      if (dbHash.empty())
        ...
        CLog::Log(LOGDEBUG, "VideoInfoScanner: Rescanning dir '%s' due to change (%s != %s)",
                  NlcUrl::GetRedacted(item->GetPath()).c_str(), dbHash.c_str(), hash.c_str());
    ...
    */
    ...
    for (int i=0;i<items.Size();++i)
    {
      ...
      if (!EnumerateEpisodeItem(items[i].get(), episodeList))
        CLog::Log(LOGDEBUG, "VideoInfoScanner: Could not enumerate file %s", NlcUrl::GetRedacted(items[i]->GetPath()).c_str());
    }
    return true;
  }
  ...
  bool CVideoInfoScanner::ProcessItemByVideoInfoTag(const CFileItem *item, EPISODELIST &episodeList)
  {
    ...
    if (isValid)
    {
      ...
      CLog::Log(LOGDEBUG, "%s - found match for: %s. Season %d, Episode %d", __FUNCTION__,
                NlcUrl::GetRedacted(episode.strPath).c_str(), episode.iSeason, episode.iEpisode);
      return true;
    }
    ...
    if (tag->m_firstAired.IsValid())
    {
      ...
      CLog::Log(LOGDEBUG, "%s - found match for: '%s', firstAired: '%s' = '%s', title: '%s'",
                __FUNCTION__, NlcUrl::GetRedacted(episode.strPath).c_str(),
    ...
    if (!tag->m_strTitle.empty())
    {
      ...
      CLog::Log(LOGDEBUG, "%s - found match for: '%s', title: '%s'", __FUNCTION__,
                NlcUrl::GetRedacted(episode.strPath).c_str(), episode.strTitle.c_str());
      return true;
    }
    ...
    if (tag->m_iSeason == 0 && tag->m_iEpisode == 0)
    {
      CLog::Log(LOGDEBUG,
                ...
                __FUNCTION__, NlcUrl::GetRedacted(item->GetPath()).c_str());
      return true;
    }
  ...
  bool CVideoInfoScanner::EnumerateEpisodeItem(const CFileItem *item, EPISODELIST& episodeList)
  {
    ...
    strLabel = NlcUrl::Decode(NlcUrl::GetRedacted(strLabel));
    ...
    for (unsigned int i=0;i<expression.size();++i)
    {
      ...
      if (byDate)
      {
        ...
        CLog::Log(LOGDEBUG, "VideoInfoScanner: Found date based match %s (%s) [%s]",
                  NlcUrl::GetRedacted(episode.strPath).c_str(), episode.cDate.GetAsLocalizedDate().c_str(),
      ...
      else if (byTitle)
      {
        ...
        CLog::Log(LOGDEBUG, "VideoInfoScanner: Found title based match %s (%s) [%s]",
                  NlcUrl::GetRedacted(episode.strPath).c_str(), episode.strTitle.c_str(), expression[i].regexp.c_str());
      }
      else
      {
        ...
        CLog::Log(LOGDEBUG, "VideoInfoScanner: Found episode match %s (s%de%d) [%s]",
                  NlcUrl::GetRedacted(episode.strPath).c_str(), episode.iSeason, episode.iEpisode,
                  expression[i].regexp.c_str());
      }
      ...
      if (!byDate && reg2.RegComp(CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_tvshowMultiPartEnumRegExp))
      {
               ...
               static_cast<int>((regexppos = reg.RegFind(remainder.c_str() + offset)) > -1))
        {
             ...
             (regexppos >= 0 && regexp2pos == -1))
          {
            ...
            CLog::Log(LOGDEBUG, "VideoInfoScanner: Adding new season %d, multipart episode %d [%s]",
                   ...
                   (regexp2pos >= 0 && regexppos == -1))
          {
            ...
            CLog::Log(LOGDEBUG, "VideoInfoScanner: Adding multipart episode %d [%s]",
                      episode.iEpisode,
                      CServiceBroker::GetSettingsComponent()
                          ->GetAdvancedSettings()
                          ->m_tvshowMultiPartEnumRegExp.c_str());
  ...
  long CVideoInfoScanner::AddVideo(CFileItem *pItem, const CONTENT_TYPE &content, bool videoFolder /* = false */, bool useLocal /* = true */, const CVideoInfoTag *showInfo /* = NULL */, bool libraryImport /* = false */)
  {
    ...
    CLog::Log(LOGDEBUG, "VideoInfoScanner: Adding new item to %s:%s", TranslateContent(content).c_str(), NlcUrl::GetRedacted(pItem->GetPath()).c_str());
    long lResult = -1;
    ...
    for (EPISODELIST::iterator file = files.begin(); file != files.end(); ++file)
    {
      ...
      if (episodes.empty())
      {
        CLog::Log(LOGERROR,
                  ...
                  NlcUrl::GetRedacted(file->strPath).c_str());
        continue;
      }
      ...
      if (!bFound)
      {
        ...
        else if (!file->strTitle.empty())
        {
          ...
          if (index >= 0 && matchscore >= minscore)
          {
            ...
            bFound = true;
            CLog::Log(LOGDEBUG,
                      "%s fuzzy title match for show: '%s', title: '%s', match: '%3.2f', score: %3.2f "
                      ">= {:f}",
      ...
      else
      {
        CLog::Log(
            LOGDEBUG,
            "%s - no match for show: '%s', season: %d, episode: %d.%d, airdate: '%s', title: '%s'",
