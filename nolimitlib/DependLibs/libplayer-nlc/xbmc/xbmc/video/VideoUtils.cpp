...
void CAsyncGetItemsForPlaylist::GetItemsForPlaylist(const std::shared_ptr<CFileItem>& item)
{
  ...
  else if (item->IsPlayList())
  {
    ...
    if (!playList)
    {
      CLog::LogF(LOGERROR, "Failed to create playlist %s", item->GetPath().c_str());
      return;
    }

    if (!playList->Load(item->GetPath()))
    {
      CLog::LogF(LOGERROR, "Failed to load playlist %s", item->GetPath().c_str());
      return;
    }
  ...
  else if (item->HasVideoInfoTag())
  {
        ...
        (mode == ContentUtils::PlayMode::CHECK_AUTO_PLAY_NEXT_ITEM && IsAutoPlayNextItem(*item)))
    {
      ...
      if (parentPath.empty())
      {
        ...
        if (parentPath.empty())
        {
          CLog::LogF(LOGERROR, "Unable to obtain parent path for '%s'", item->GetPath().c_str());
          return;
        }
      }
