...
std::unique_ptr<CTextureBase> CVideoEmbeddedImageFileLoader::Load(std::string specialType,
                                                              std::string filePath,
                                                              ...
                                                              unsigned int preferredHeight) const
{
  ...
  if (GetEmbeddedThumb(filePath, specialType.substr(6), art))
    return CTextureBase::LoadFromFileInMemory(art.m_data.data(), art.m_size, art.m_mime, preferredWidth,
                                          preferredHeight);
  return {};
}
