...
int CVideoDatabase::RunQuery(const std::string &sql)
{
  ...
  CLog::Log(LOGDEBUG, LOGDATABASE, "%s took %d ms for %d items query: %s", __FUNCTION__,
            duration.count(), rows, sql.c_str());
...
void CVideoDatabase::UpdateFileDateAdded(CVideoInfoTag& details)
{
  ...
  {
    CLog::Log(LOGERROR, "%s(%s, %s) failed", __FUNCTION__, NlcUrl::GetRedacted(details.GetPath()).c_str(),
...
bool CVideoDatabase::LinkMovieToTvshow(int idMovie, int idShow, bool bRemove)
{
  ...
  {
    CLog::Log(LOGERROR, "%s (%d, %s) failed", __FUNCTION__, idMovie, idShow);
  }
...
bool CVideoDatabase::IsLinkedToTvshow(int idMovie)
{
  ...
  {
    CLog::Log(LOGERROR, "%s (%d) failed", __FUNCTION__, idMovie);
  }
...
int CVideoDatabase::GetMovieId(const std::string& strFilenameAndPath)
{
  try
  {
    ...
    CLog::Log(LOGDEBUG, LOGDATABASE, "%s (%s), query = %s", __FUNCTION__,
              NlcUrl::GetRedacted(strFilenameAndPath).c_str(), strSQL.c_str());
    m_pDS->query(strSQL);
    if (m_pDS->num_rows() > 0)
...
int CVideoDatabase::AddRatings(int mediaId, const char *mediaType, const RatingMap& values, const std::string& defaultRating)
{
  ...
  {
    CLog::Log(LOGERROR, "%s (%d - %s) failed", __FUNCTION__, mediaId, mediaType);
  }
...
int CVideoDatabase::UpdateUniqueIDs(int mediaId, const char *mediaType, const CVideoInfoTag& details)
{
  ...
  {
    CLog::Log(LOGERROR, "%s unable to update unique ids of (%d)", __FUNCTION__, mediaType);
  }
  return -1;
}
...
int CVideoDatabase::UpdateDetailsForMovie(int idMovie, CVideoInfoTag& details, const std::map<std::string, std::string> &artwork, const std::set<std::string> &updatedDetails)
{
  ...
  try
  {
    ...
    CLog::Log(LOGINFO, "%s: Finished updates for movie %s", __FUNCTION__, idMovie);
...
void CVideoDatabase::DeleteBookMarkForEpisode(const CVideoInfoTag& tag)
{
  ...
  {
    CLog::Log(LOGERROR, "%s (%d) failed", __FUNCTION__, tag.m_iDbId);
  }
}
...
void CVideoDatabase::DeleteTag(int idTag, VideoDbContentType mediaType)
{
  ...
  {
    CLog::Log(LOGERROR, "%s (%d) failed", __FUNCTION__, idTag);
  }
}
...
bool CVideoDatabase::GetStreamDetails(CVideoInfoTag& tag) const
{
  ...
  {
    CLog::Log(LOGERROR, "%s(%d) failed", __FUNCTION__, tag.m_iFileId);
  }
...
void CVideoDatabase::GetCast(int media_id, const std::string &media_type, std::vector<SActorInfo> &cast)
{
  ...
  {
    CLog::Log(LOGERROR, "%s(%d,%s) failed", __FUNCTION__, media_id, media_type.c_str());
  }
}
                                      ...
                                      VideoDbContentType iType)
{
  try
  {
    ...
    if (iType == VideoDbContentType::MOVIES)
    {
      CLog::Log(LOGINFO, "Changing Movie:id:%d New Title:%s", idMovie, strNewMovieTitle.c_str());
      content = MediaTypeMovie;
    }
    else if (iType == VideoDbContentType::EPISODES)
    {
      CLog::Log(LOGINFO, "Changing Episode:id:%d New Title:%s", idMovie, strNewMovieTitle.c_str());
      content = MediaTypeEpisode;
    }
    else if (iType == VideoDbContentType::TVSHOWS)
    {
      CLog::Log(LOGINFO, "Changing TvShow:id:%d New Title:%s", idMovie, strNewMovieTitle.c_str());
      content = MediaTypeTvShow;
    }
    else if (iType == VideoDbContentType::MUSICVIDEOS)
    {
      CLog::Log(LOGINFO, "Changing MusicVideo:id:%d New Title:%s", idMovie, strNewMovieTitle.c_str());
      content = MediaTypeMusicVideo;
    }
    else if (iType == VideoDbContentType::MOVIE_SETS)
    {
      CLog::Log(LOGINFO, "Changing Movie set:id:%s New Title:%s", idMovie, strNewMovieTitle.c_str());
  ...
  catch (...)
  {
    CLog::Log(
        LOGERROR,
        "%s (int idMovie, const std::string& strNewMovieTitle) failed on MovieID:%s and Title:%s",
                                          ...
                                          VideoDbContentType iType /* = MOVIES */)
{
  ...
  {
    CLog::Log(LOGERROR,
              "%s (int idDb, const std::string& strNewSortTitle, VIDEODB_CONTENT_TYPE iType) "
              "failed on ID: %d and Sort Title: %s",
...
void CVideoDatabase::EraseVideoSettings(const CFileItem &item)
{
  ...
  try
  {
    ...
    CLog::Log(LOGINFO, "Deleting settings information for files %s",
              NlcUrl::GetRedacted(item.GetPath()).c_str());
    m_pDS->exec(sql);
  }
  catch (...)
  {
          ...
          if (overwrite || !CFile::Exists(nfoFile, false))
          {
            if(!xmlDoc.SaveFile(nfoFile))
            {
              CLog::Log(LOGERROR, "%s: Musicvideo nfo export failed! ('%s')", __FUNCTION__,
                        nfoFile.c_str());
              CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Error, g_localizeStrings.Get(20302), nfoFile.c_str());
              iFailCount++;
            }
          }
        }
...
bool CVideoDatabase::SetVideoUserRating(int dbId, int rating, const MediaType& mediaType)
{
  ...
  {
    CLog::Log(LOGERROR, "%s (%d, %s, %d) failed", __FUNCTION__, dbId, mediaType.c_str(), rating);
  }
  return false;
}
