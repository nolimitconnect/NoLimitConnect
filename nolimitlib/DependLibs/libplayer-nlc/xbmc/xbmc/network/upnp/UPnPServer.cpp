                                    ...
                                    const PLT_HttpRequestContext& context)
{
    ...
    if (!load) {
        ...
        unsigned int time = XbmcThreadsNoChrono::SystemClockMillis();
        ...
        if (items.CacheToDiscAlways() || (items.CacheToDiscIfSlow() && (XbmcThreadsNoChrono::SystemClockMillis() - time) > 1000 )) {
            NPT_AutoLock lock(m_CacheMutex);
            items.Save();
        }
    }
