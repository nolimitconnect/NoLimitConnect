...
void CNetworkBase::NetworkMessage( EMESSAGE message, int param )
{
    ...
    case SERVICES_UP:
        ...
        CLog::Log( LOGDEBUG, "%s - NOT Starting network services", __FUNCTION__ );
#else
    ...
    case SERVICES_DOWN:
#if defined(ENABLE_NLC_PLAYER)
        CLog::Log( LOGDEBUG, "%s - network services is stopped in Player NLC mode", __FUNCTION__ );
#else
...
{
#if defined(ENABLE_NLC_PLAYER)
    CLog::Log( LOGDEBUG, "%s - disabled in Player NLC mode", __FUNCTION__ );
    return false;
#endif // defined(ENABLE_NLC_PLAYER)
    int i, j, packet;
