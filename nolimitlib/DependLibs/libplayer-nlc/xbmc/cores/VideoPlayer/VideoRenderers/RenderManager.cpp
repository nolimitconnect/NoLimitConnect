...
void CRenderManager::PreInit()
{
    ...
    if( !g_application.IsCurrentThread() )
    {
        ...
        if( !m_initEvent.WaitMSec( 4000 ) )
        {
            CLog::Log( LOGERROR, "%s - timed out waiting for renderer to preinit", __FUNCTION__ );

            g_application.toGuiMediaError( eMediaErrorPlayerInit, "Render PreInit Timeout" );
        }
...
void CRenderManager::Render( bool clear, DWORD flags, DWORD alpha, bool gui )
{
    ...
    if( gui )
    {
        ...
        //LogModule( eLogVideoIo, LOG_DEBUG, "CRenderManager::Render m_presentsource %d dest %dx%d view %dx%d",
        ...
        if( m_renderDebug )
        {
            ...
            int missedvblanks;
            info.vsync = StringUtils::Format( "VSyncOff: %.1f latency: %.3f  ", m_clockSync.m_syncOffset / 1000, DVD_TIME_TO_MSEC( m_displayLatency ) / 1000.0f );
