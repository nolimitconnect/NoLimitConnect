...
bool movieStarted = false; // temp for debugging
...
void CVideoPlayerAudio::OpenStream( CDVDStreamInfo& hints, std::unique_ptr<CDVDAudioCodec> codec )
{
    ...
    if( m_synctype == SYNC_DISCON )
        LogModule( eLogPlayerNlc, LOG_INFO, "Allowing max Out-Of-Sync Value of %3.3f ms", m_disconAdjustTimeMs );
...
void CVideoPlayerAudio::CloseStream( bool bWaitForBuffers )
{
    ...
    LogModule( eLogPlayerNlc, LOG_DEBUG, "Waiting for audio thread to exit" );

    // shut down the adio_decode thread and wait for it
    StopThread(); // will set this->m_bStop to true

    // destroy audio device
    LogModule( eLogPlayerNlc, LOG_DEBUG, "Closing audio device" );
    if( bWait )
    {
        m_bStop = false;
        m_audioSink.Drain();
    ...
    LogModule( eLogPlayerNlc, LOG_DEBUG, "Deleting audio codec" );
    if( m_pAudioCodec )
    {
...
{
    CLog::Log( LOGNOTICE, "running thread: CVideoPlayerAudio::Process() thread %d", VxGetCurrentThreadId() );
    ...
    while( !m_bStop )
    {
        ...
        if( MSGQ_IS_ERROR( ret ) )
        {
            if( !m_messageQueue.ReceivedAbortRequest() )
                CLog::Log( LOGERROR, "MSGQ_IS_ERROR returned true (%d)", ret );
        ...
        { //player asked us to set internal clock
            ...
            CLog::Log( LOGDEBUG,
                       "CVideoPlayerAudio - CDVDMsg::GENERAL_RESYNC(%3.3f), level: %d, cache: %3.3f", pts,
...
bool CVideoPlayerAudio::ProcessDecoderOutput( DVDAudioFrame& audioframe )
{
    ...
    if( m_synctype == SYNC_DISCON )
    {
        ...
        if( m_disconLearning )
        {
            ...
            if( m_disconTimer.IsTimePast() )
            {
                ...
                LogModule( eLogPlayerNlc, LOG_DEBUG, "Changed max allowed Out-Of-Sync value to %d ms due self-learning",
    ...
    if( m_syncState == IDVDStreamPlayer::SYNC_STARTING )
    {
        ...
        LogModule( eLogAudioIo, LOG_VERBOSE, " CVideoPlayerAudio SYNC_STARTING cachetime %3.3f cachetotal %3.3f percent %d",
        ...
        if( cachetime >= cachetotal * 0.75 )
        {
            ...
            LogModule( eLogAudioIo, LOG_VERBOSE, " CVideoPlayerAudio PLAYER_STARTED & PLAYER_AVCHANGE codec %s because cache >= 75 percent",
                       m_pAudioCodec->GetName().c_str() );
            m_messageParent.Put( std::make_shared<CDVDMsg>( CDVDMsg::PLAYER_AVCHANGE ) );
        }
    }
