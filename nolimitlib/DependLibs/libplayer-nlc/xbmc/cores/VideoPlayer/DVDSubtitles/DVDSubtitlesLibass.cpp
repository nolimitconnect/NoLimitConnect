...
{
  CLog::Log(LOGINFO, "CDVDSubtitlesLibass: Using libass version 0x%X", ass_library_version());
...
void CDVDSubtitlesLibass::Configure()
{
  ...
  for (const auto& item : items)
  {
    ...
    {
      CLog::LogF(LOGERROR, "Failed to load file %s", filepath.c_str());
      continue;
    }
  ...
  {
    LogModule( eLogPlayerNlc, LOG_FATAL,
               "The application font %s is missing. The default subtitle font cannot be set.",
