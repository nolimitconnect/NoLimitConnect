...
CVideoPlayer::CVideoPlayer( IPlayerCallback& callback )
    : m_Nlc( INlcRender::getINlcRender() )
    , IPlayer( callback )
    , CThread( "VideoPlayer" ),
...
bool CVideoPlayer::OpenDemuxStream()
{
    ...
    if( !m_pDemuxer )
    {
        ...
        g_application.toGuiMediaError( eMediaErrorPlayerInit, "Error creating demuxer" );
        return false;
    }
...
void CVideoPlayer::Process()
{
    ...
    while( !m_bAbortRequest )
    {
        ...
        if( m_demuxerSpeed != DVD_PLAYSPEED_PAUSE )
        {
            ...
            {
                fillBuffer = true;
                //LogModule( eLogVideoIo, LOG_DEBUG, "CVideoPlayer::Process fill video %d audio %d", videoLevel, audioLevel );
            }
            else
            {
                //LogModule( eLogVideoIo, LOG_DEBUG, "CVideoPlayer::Process NO fill video %d audio %d", videoLevel, audioLevel );
            }
        ...
        if( !pPacket )
        {
            ...
            {
                m_VideoPlayerVideo->SendDvdMessage( std::make_shared<CDVDMsg>( CDVDMsg::VIDEO_DRAIN ) );
            }
...
void CVideoPlayer::ProcessAudioData( CDemuxStream* pStream, DemuxPacket* pPacket )
{
    ...
    m_VideoPlayerAudio->SendDvdMessage( std::make_shared<CDVDMsgDemuxerPacket>( pPacket, drop ) );
...
void CVideoPlayer::ProcessVideoData( CDemuxStream* pStream, DemuxPacket* pPacket )
{
    ...
    m_VideoPlayerVideo->SendDvdMessage( std::make_shared<CDVDMsgDemuxerPacket>( pPacket, drop ) );
...
void CVideoPlayer::ProcessSubData( CDemuxStream* pStream, DemuxPacket* pPacket )
{
    ...
    m_VideoPlayerSubtitle->SendDvdMessage( std::make_shared<CDVDMsgDemuxerPacket>( pPacket, drop ) );
}
...
void CVideoPlayer::ProcessTeletextData( CDemuxStream* pStream, DemuxPacket* pPacket )
{
    ...
    m_VideoPlayerTeletext->SendDvdMessage( std::make_shared<CDVDMsgDemuxerPacket>( pPacket, drop ) );
}
...
void CVideoPlayer::ProcessAudioID3Data( CDemuxStream* pStream, DemuxPacket* pPacket )
{
    ...
    m_VideoPlayerAudioID3->SendDvdMessage( std::make_shared<CDVDMsgDemuxerPacket>( pPacket, drop ) );
}
...
void CVideoPlayer::HandlePlaySpeed()
{
        ...
        (m_CurrentAudio.syncState == IDVDStreamPlayer::SYNC_WAITSYNC) )
    {
              ...
              m_CurrentVideo.syncState == IDVDStreamPlayer::SYNC_INSYNC) )
        {
            m_CurrentAudio.syncState = IDVDStreamPlayer::SYNC_INSYNC;
            m_CurrentAudio.avsync = CCurrentStream::AV_SYNC_NONE;
            m_VideoPlayerAudio->SendDvdMessage(
                   ...
                   m_CurrentAudio.syncState == IDVDStreamPlayer::SYNC_INSYNC) )
        {
            m_CurrentVideo.syncState = IDVDStreamPlayer::SYNC_INSYNC;
            m_CurrentVideo.avsync = CCurrentStream::AV_SYNC_NONE;
            m_VideoPlayerVideo->SendDvdMessage(
                std::make_shared<CDVDMsgDouble>( CDVDMsg::GENERAL_RESYNC, m_clock.GetClock() ), 1 );
        }
        else if( video && audio )
        {
                ...
                m_playSpeed == DVD_PLAYSPEED_PAUSE )
            {
                ...
                LogModule( eLogVideoIo, LOG_DEBUG, "VideoPlayer::Sync1: clock %3.3f", clock );
            }
            else if( m_CurrentAudio.starttime != DVD_NOPTS_VALUE && m_CurrentAudio.packets > 0 )
            {
                if( m_pInputStream->IsRealtime() )
                {
                    ...
                    LogModule( eLogVideoIo, LOG_DEBUG, "VideoPlayer::Sync2: clock %3.3f", clock );
                }
                else
                {
                    clock = m_CurrentAudio.starttime - m_CurrentAudio.cachetime;
                    LogModule( eLogVideoIo, LOG_DEBUG, "VideoPlayer::Sync3: clock %3.3f", clock );
                }
                ...
                if( m_CurrentVideo.starttime != DVD_NOPTS_VALUE && (m_CurrentVideo.packets > 0) )
                {
                    if( m_CurrentVideo.starttime - m_CurrentVideo.cachetotal < clock )
                    {
                        ...
                        LogModule( eLogVideoIo, LOG_DEBUG, "VideoPlayer::Sync4: clock %3.3f", clock );
                    }
                             ...
                             !m_pInputStream->IsRealtime() )
                    {
                        ...
                        {
                            clock = maxAudioTime;
                            LogModule( eLogVideoIo, LOG_DEBUG, "VideoPlayer::Sync5: clock %3.3f", clock );
                        }
                        else
                        {
                            clock = m_CurrentVideo.starttime - m_CurrentVideo.cachetotal;
                            LogModule( eLogVideoIo, LOG_DEBUG, "VideoPlayer::Sync6: clock %3.3f", clock );
                        }
                    }
                }
            }
            ...
            {
                clock = m_CurrentVideo.starttime - m_CurrentVideo.cachetotal;
                LogModule( eLogVideoIo, LOG_DEBUG, "VideoPlayer::Sync7: clock %3.3f", clock );
            }
    ...
    if( edit.action == EDL::Action::CUT )
    {
            ...
            (m_playSpeed < 0 && correctClock < (edit.end - 1000)) )
        {
            CLog::Log( LOGDEBUG, "%s - Clock in EDL cut [%s - %s]: %s. Automatically skipping over.",
                       __FUNCTION__, CEdl::MillisecondsToTimeString( edit.start ).c_str(),
                       CEdl::MillisecondsToTimeString( edit.end ).c_str(), CEdl::MillisecondsToTimeString( clock ).c_str() );
        ...
        if( m_playSpeed >= 0 && m_Edl.GetLastEditTime() != edit.start && clock < edit.end - 1000 )
        {
            ...
            if( m_SkipCommercials )
            {
                CLog::Log( LOGDEBUG, "%s - Clock in commercial break [%s - %s]: %s. Automatically skipping to end of commercial break",
                           __FUNCTION__, CEdl::MillisecondsToTimeString( edit.start ).c_str(),
                           CEdl::MillisecondsToTimeString( edit.end ).c_str(),
...
void CVideoPlayer::SendPlayerMessage( std::shared_ptr<CDVDMsg> pMsg, unsigned int target )
{
    ...
    if( player )
        player->SendDvdMessage( std::move( pMsg ), 0 );
}

void CVideoPlayer::OnExit()
{
    LogModule( eLogPlayerNlc, LOG_DEBUG, "CVideoPlayer::OnExit()" );
    ...
    // close each stream
    if( !m_bAbortRequest )
         LogModule( eLogPlayerNlc, LOG_DEBUG, "VideoPlayer: eof, waiting for queues to empty" );
    ...
    while( m_messenger.Get( pMsg, 0ms ) == MSGQ_OK )
    {
        ...
        else if( pMsg->IsType( CDVDMsg::PLAYER_STARTED ) )
        {
            ...
            if( msg.player == VideoPlayer_AUDIO )
            {
                ...
                CLog::Log( LOGDEBUG, "CVideoPlayer::HandleMessages - AUDIO player started %d", msg.player );
            }
            if( msg.player == VideoPlayer_VIDEO )
            {
                ...
                CLog::Log( LOGDEBUG, "CVideoPlayer::HandleMessages - VIDEO player started %d", msg.player );
            }
        }
        else if( pMsg->IsType( CDVDMsg::PLAYER_REPORT_STATE ) )
        {
            ...
            if( msg.player == VideoPlayer_AUDIO )
            {
                ...
                CLog::Log( LOGDEBUG, "CVideoPlayer::HandleMessages - AUDIO player %d reported state: %d", msg.player, msg.syncState );
            }
            if( msg.player == VideoPlayer_VIDEO )
            {
                m_CurrentVideo.syncState = msg.syncState;
                CLog::Log( LOGDEBUG, "CVideoPlayer::HandleMessages - VIDEO player %d reported state: %d", msg.player, msg.syncState );
            }
        }
...
void CVideoPlayer::SetCaching( ECacheState state )
{
    ...
    if( state == 0 )
    {
        LogModule( eLogVideoIo, LOG_VERBOSE, " CVideoPlayer::SetCaching - CACHESTATE_DONE" );
    }
    else if( state == 1 )
    {
        LogModule( eLogVideoIo, LOG_VERBOSE, " CVideoPlayer::SetCaching - CACHESTATE_FULL" );
    }
    else if( state == 2 )
    {
        LogModule( eLogVideoIo, LOG_VERBOSE, " CVideoPlayer::SetCaching - CACHESTATE_INIT" );
    }
    else if( state == 3 )
    {
        LogModule( eLogVideoIo, LOG_VERBOSE, " CVideoPlayer::SetCaching - CACHESTATE_PLAY" );
    }
    else if( state == 4 )
    {
        LogModule( eLogVideoIo, LOG_VERBOSE, " CVideoPlayer::SetCaching - CACHESTATE_FLUSH" );
    }
...
bool CVideoPlayer::OpenAudioStream( CDVDStreamInfo& hint, bool reset )
{
        ...
        m_CurrentAudio.hint != hint )
    {
        ...
        player->SendDvdMessage( std::make_shared<CDVDMsgBool>( CDVDMsg::GENERAL_PAUSE, m_displayLost ), 1 );
        ...
        player->SendDvdMessage( std::make_shared<CDVDMsg>( CDVDMsg::GENERAL_RESET ), 0 );

    m_HasAudio = true;

    static_cast<IDVDStreamPlayerAudio*>(player)->SendDvdMessage(
...
bool CVideoPlayer::OpenVideoStream( CDVDStreamInfo& hint, bool reset )
{
    ...
    static_cast<IDVDStreamPlayerVideo*>(player)->SendDvdMessage(
        std::make_shared<CDVDMsg>( CDVDMsg::PLAYER_REQUEST_STATE ), 1 );
...
{
    LogModule( eLogVideoIo, LOG_ERROR, "Attempted to open radio stream but radio is disabled" );
    return false;
}
...
bool CVideoPlayer::CloseStream( CCurrentStream& current, bool bWaitForBuffers )
{
    ...
    LogModule( eLogPlayerNlc, LOG_DEBUG, " CVideoPlayer::CloseStream %d", current.id );
...
void CVideoPlayer::FlushBuffers( double pts, bool accurate, bool sync )
{
        ...
        m_playSpeed == DVD_PLAYSPEED_PAUSE )
    {
        ...
        m_VideoPlayerAudio->SendDvdMessage( msg, 1 );
        m_VideoPlayerVideo->SendDvdMessage( msg, 1 );
        msg->Wait( m_bStop, 0 );
  ...
  { \
    ...
    if( pMenus )
    {
        ...
        if( pMenus->IsInMenu() )
        {
            ...
            {
                //if( action.GetID() == action.GetID() )
                LogMsg( LOG_VERBOSE, "%s mouse action %d", __func__, action.GetID() );
                CRect rs, rd, rv;
                ...
                if( action.GetID() == ACTION_MOUSE_LEFT_CLICK )
                {
                    ...
                    {
                        LogMsg( LOG_VERBOSE, "%s pMenus->OnMouseClick", __func__ );
                        return true;
                    }
                    else
                    {
                        LogMsg( LOG_VERBOSE, "%s ACTION_TRIGGER_OSD", __func__ );
                        CServiceBroker::GetAppMessenger()->PostMsg( TMSG_GUI_ACTION, WINDOW_INVALID, -1, static_cast<void*>(new CAction( ACTION_TRIGGER_OSD )) );
                        return false;
                    }
                }
...
{
    if( m_VideoPlayerVideo->getIsPlayerQueueFull() )
    {
        LogModule( eLogVideoIo, LOG_VERBOSE, " CVideoPlayer::VideoParamsChange PLAYER_AVCHANGE" );
    }
...
void CVideoPlayer::OnLostDisplay()
{
    ...
    m_VideoPlayerAudio->SendDvdMessage( std::make_shared<CDVDMsgBool>( CDVDMsg::GENERAL_PAUSE, true ), 1 );
    m_VideoPlayerVideo->SendDvdMessage( std::make_shared<CDVDMsgBool>( CDVDMsg::GENERAL_PAUSE, true ), 1 );
    m_clock.Pause( true );
    m_displayLost = true;
    FlushRenderer();
}
...
void CVideoPlayer::OnResetDisplay()
{
    ...
    m_VideoPlayerAudio->SendDvdMessage( std::make_shared<CDVDMsgBool>( CDVDMsg::GENERAL_PAUSE, false ), 1 );
    m_VideoPlayerVideo->SendDvdMessage( std::make_shared<CDVDMsgBool>( CDVDMsg::GENERAL_PAUSE, false ), 1 );
    m_clock.Pause( false );
    m_displayLost = false;
    m_VideoPlayerAudio->SendDvdMessage( std::make_shared<CDVDMsg>( CDVDMsg::PLAYER_DISPLAY_RESET ), 1 );
}
...
void CVideoPlayer::UpdateFileItemStreamDetails( CFileItem& item )
{
    ...
    LogModule( eLogPlayerNlc, LOG_DEBUG, "CVideoPlayer: updating file item stream details with available streams" );
