...
void CActiveAE::Process()
{
    ...
    while( !m_bStop )
    {
        ...
        {
            msg = m_controlPort.GetProtocolMessage();
...
//    }
	...
	{
		inputFormat.m_dataFormat = AE_FMT_FLOAT;
				inputFormat.m_sampleRate = AUDIO_SAMPLE_RATE_KODI;
		inputFormat.m_channelLayout.setTo2ChannelLayout();
		inputFormat.m_frameSize = AUDIO_CHANNELS_KODI * sizeof( float );
		// frames is misnamed.. is really number of samples per frame 
		inputFormat.m_frames = AUDIO_SAMPLE_RATE_KODI * AUDIO_SEC_KODI;  // 20 ms frame of samples
