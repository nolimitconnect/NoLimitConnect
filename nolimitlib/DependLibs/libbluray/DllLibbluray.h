...
public:
    ...
    uint32_t bd_get_titles( BLURAY *p1, uint8_t p2, uint32_t p3 )  override;
    BLURAY_TITLE_INFO* bd_get_title_info( BLURAY *p1, uint32_t p2, unsigned p3 )  override;
    BLURAY_TITLE_INFO* bd_get_playlist_info( BLURAY *p1, uint32_t p2, unsigned p3 )  override;
    void bd_free_title_info( BLURAY_TITLE_INFO *p1 )  override;
    BLURAY* bd_open( const char* p1, const char* p2 )  override;
    int bd_open_disc( BLURAY *p1, const char *p2, const char *p3 )  override;
    int bd_open_stream( BLURAY *p1, void *p2, read_blocks_f p3 )  override;
    int bd_open_files( BLURAY *p1, void *p2, open_dir_f p3, open_file_f p4 )  override;
    BLURAY* bd_init( )  override;
    void bd_close( BLURAY *p1 )  override;
    int64_t bd_seek( BLURAY *p1, uint64_t p2 )   override;
    int64_t bd_seek_time( BLURAY *p1, uint64_t p2 )  override;
    int bd_read( BLURAY *p1, unsigned char *p2, int p3 )  override;
    int64_t bd_seek_chapter( BLURAY *p1, unsigned p2 )  override;
    int64_t bd_chapter_pos( BLURAY *p1, unsigned p2 )   override;
    uint32_t bd_get_current_chapter( BLURAY *p1 )  override;
    int64_t bd_seek_mark( BLURAY *p1, unsigned p2 )  override;
    int bd_select_playlist( BLURAY *p1, uint32_t p2 )   override;
    int bd_select_title( BLURAY *p1, uint32_t p2 )   override;
    int bd_select_angle( BLURAY *p1, unsigned p2 )  override;
    void bd_seamless_angle_change( BLURAY *p1, unsigned p2 )  override;
    uint64_t bd_get_title_size( BLURAY *p1 )  override;
    uint32_t bd_get_current_title( BLURAY *p1 )  override;
    unsigned bd_get_current_angle( BLURAY *p1 )  override;
    uint64_t bd_tell( BLURAY *p1 )  override;
     uint64_t bd_tell_time( BLURAY *p1 )  override;
    BD_FILE_OPEN bd_register_file( BD_FILE_OPEN p1 )  override;
    BD_DIR_OPEN bd_register_dir( BD_DIR_OPEN p1 )  override;

    void bd_set_debug_handler( BD_LOG_FUNC p1 )  override;
    void bd_set_debug_mask( uint32_t p1 )  override;
    uint32_t bd_get_debug_mask( )  override;
    const BLURAY_DISC_INFO* bd_get_disc_info( BLURAY *p1 )  override;

    int bd_get_event( BLURAY *p1, BD_EVENT *p2 )  override;
    int bd_play( BLURAY *p1 )  override;
    int bd_read_ext( BLURAY *p1, unsigned char *p2, int p3, BD_EVENT *p4 )  override;
    int bd_read_skip_still( BLURAY *p1 )  override;
    int bd_user_input( BLURAY *p1, int64_t p2, uint32_t p3 )  override;
    int bd_set_player_setting( BLURAY *p1, uint32_t p2, uint32_t p3 )  override;
    int bd_set_player_setting_str( BLURAY *p1, uint32_t p2, const char *p3 )  override;
    void bd_register_overlay_proc( BLURAY *p1, void *p2, bd_overlay_proc_f p3 )  override;
#ifdef HAVE_LIBBLURAY_BDJ
    void bd_register_argb_overlay_proc( BLURAY *p1, void *p2, bd_argb_overlay_proc_f p3, struct bd_argb_buffer_s *p4 )  override;
#endif
    int bd_menu_call( BLURAY *p1, int64_t p2 )  override;
    int bd_mouse_select( BLURAY *p1, int64_t p2, uint16_t p3, uint16_t p4 )  override;
    int bd_get_sound_effect( BLURAY *p1, unsigned p2, struct bd_sound_effect* p3 )  override;
