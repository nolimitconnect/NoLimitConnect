...
class BlobTxSession : public BlobXferSession
{
public:
	BlobTxSession( P2PEngine& engine );
	BlobTxSession( P2PEngine& engine, std::shared_ptr<VxSktBase>& sktBase, VxNetIdent* netIdent );
	BlobTxSession( P2PEngine& engine, VxGUID& lclSessionId, std::shared_ptr<VxSktBase>& sktBase, VxNetIdent* netIdent );
