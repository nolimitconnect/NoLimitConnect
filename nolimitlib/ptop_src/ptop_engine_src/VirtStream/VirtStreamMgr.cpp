...
int VirtStreamMgr::virtFileError(VFile* fp)
{
	...
	m_LiveStream.updateIsConnected();
...
bool VirtStreamMgr::fromGuiPlayStream( AssetBaseInfo& assetInfo, VxGUID lclSessionId, int pos0to100000 )
{
	...
	m_LiveStream.m_SktBase = m_Engine.getConnectIdListMgr().findBestUserOnlineConnection( assetInfo.getDestUserId() );
	if( m_LiveStream.m_SktBase && m_LiveStream.m_SktBase->isConnected() )
	{
		m_Engine.getPluginMgr().getPlugin( ePluginTypeFileShareClient );
	}
...
//============================================================================
void VirtStreamMgr::onFileXferPktRxed( VxPktHdr* pktHdr )
{
