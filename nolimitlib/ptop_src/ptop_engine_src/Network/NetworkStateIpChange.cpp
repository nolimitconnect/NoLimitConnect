...
//============================================================================
NetworkStateIpChange::NetworkStateIpChange( bool ipv6, NetworkStateMachine& stateMachine,  std::string oldIp, std::string newIp )
...
//============================================================================
void NetworkStateIpChange::enterNetworkState( void )
{
    LogModule( eLogNetworkState, LOG_STATUS, "NetworkStateIpChange start" );

	m_Engine.getToGui().toGuiNetworkState( eNetworkStateTypeAvail );
}

//============================================================================
void NetworkStateIpChange::runNetworkState( void )
{
    VxTimer availTimer;
    LogModule( eLogNetworkState, LOG_VERBOSE, "111 NetworkStateIpChange::runNetworkState start" ); 
 ...
 //   }
    ...
    //{
	//LogMsg( LOG_INFO, "111 NetworkStateIpChange::runNetworkState resolving urls time %3.3f\n", availTimer.elapsedSec() );
    //}
		...
		&& ( false == m_Engine.getPeerMgr().isReadyToAcceptConnections( false ) ) )
	{
		...
		if( waitForListenCnt > 5 )
		{
            if( IsLogEnabled( eLogNetworkState ) )
            {
                LogMsg( LOG_ERROR, "NetworkStateIpChange::runNetworkState timed out waiting for isReadyToAcceptConnections" );
                char timeoutMsg[128];
	...
	{
        LogModule( eLogNetworkState, LOG_INFO, "NetworkStateIpChange::runNetworkState Starting UPNP %3.3f", availTimer.elapsedSec() );
	...
	if( m_DirectConnectTester.isTestResultCanDirectConnect() )
	{
		...
		{
            LogModule( eLogNetworkState, LOG_STATUS, "NetworkStateIpChange::runNetworkState eFirewallTestAssumeFirewalled %3.3f", availTimer.elapsedSec() );
            m_NetworkStateMachine.setPktAnnounceWithCanDirectConnect( directConnectTestResults.m_MyIpAddr, true );
			m_NetworkStateMachine.changeNetworkState( eNetworkStateTypeWaitForRelay );
		}
		else
		{
            LogModule( eLogNetworkState, LOG_STATUS, "NetworkStateIpChange::runNetworkState announce with direct connect %3.3f", availTimer.elapsedSec() );
	...
	else
	{
		...
		if( eFirewallTestUrlConnectionTest == firewallTestType )
		{
            LogModule( eLogNetworkState, LOG_STATUS, "NetworkStateIpChange::runNetworkState relay search %3.3f", availTimer.elapsedSec() );
            m_NetworkStateMachine.setPktAnnounceWithCanDirectConnect( directConnectTestResults.m_MyIpAddr, true );
			m_NetworkStateMachine.changeNetworkState( eNetworkStateTypeWaitForRelay );
		}
		else if( eFirewallTestAssumeNoFirewall == firewallTestType )
		{
            LogModule( eLogNetworkState, LOG_STATUS, "NetworkStateIpChange::runNetworkState assume no firewall %3.3f", availTimer.elapsedSec() );
            m_NetworkStateMachine.setPktAnnounceWithCanDirectConnect( directConnectTestResults.m_MyIpAddr, false );
			m_Engine.getToGui().toGuiUpdateMyIdent( &m_PktAnn );
			m_NetworkStateMachine.changeNetworkState( eNetworkStateTypeOnlineDirect );
            canDirectConnect = true;
		...
		{
            LogModule( eLogNetworkState, LOG_STATUS, "NetworkStateIpChange::runNetworkState assume firewall %3.3f", availTimer.elapsedSec() );
            m_NetworkStateMachine.setPktAnnounceWithCanDirectConnect( directConnectTestResults.m_MyIpAddr, true );
			m_NetworkStateMachine.changeNetworkState( eNetworkStateTypeWaitForRelay );
		}
		else
