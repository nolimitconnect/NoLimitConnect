...
: m_FormatTag( 1 )
, m_Channels( 1 )
, m_SamplesPerSec( ECHO_SAMPLE_RATE )
, m_AvgBytesPerSec( ECHO_SAMPLE_RATE * AUDIO_BYTES_PER_SAMPLE )
...
, m_StreamFormat(  )
{
	...
	m_StreamHdr.setRate( ECHO_SAMPLE_RATE );
	m_StreamHdr.setScale( 1 );
	m_StreamHdr.m_Length = AUDIO_BUF_SIZE;
...
, m_JunkLen( sizeof( AviVidJunk )  - 8 )
{
	...
	strcpy( (char *)m_JunkData, "Created by No Limit Connect at https://nolimitconnect.org" );
}
